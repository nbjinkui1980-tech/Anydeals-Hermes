...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      anydeals-agent = self'.packages.default;
      anydealsVenv = anydeals-agent.anydealsVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "anydeals-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${anydealsVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "anydeals-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "anydeals-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${anydeals-agent}/bin/anydeals || (echo "FAIL: anydeals binary missing"; exit 1)
          test -x ${anydeals-agent}/bin/anydeals-agent || (echo "FAIL: anydeals-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${anydeals-agent}/bin/anydeals version 2>&1 | grep -qi "anydeals" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "anydeals-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in anydeals anydeals-agent anydeals-acp; do
            test -x ${anydeals-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "anydeals-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking anydeals --help ==="
          ${anydeals-agent}/bin/anydeals --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${anydeals-agent}/bin/anydeals --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "anydeals-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${anydeals-agent}/share/anydeals-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${anydeals-agent}/share/anydeals-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ANYDEALS_BUNDLED_SKILLS" ${anydeals-agent}/bin/anydeals || \
            (echo "FAIL: ANYDEALS_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ANYDEALS_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "anydeals-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${anydeals-agent}/share/anydeals-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${anydeals-agent}/share/anydeals-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ANYDEALS_BUNDLED_PLUGINS" ${anydeals-agent}/bin/anydeals || \
            (echo "FAIL: ANYDEALS_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ANYDEALS_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "anydeals-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${anydeals-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${anydeals-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ANYDEALS_TUI_DIR" ${anydeals-agent}/bin/anydeals || \
            (echo "FAIL: ANYDEALS_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ANYDEALS_TUI_DIR set in wrapper"
        ...
        # Verify ANYDEALS_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        anydeals-node = pkgs.runCommand "anydeals-node-version" { } ''
          set -e
          echo "=== Checking ANYDEALS_NODE in wrapper ==="
          grep -q "ANYDEALS_NODE" ${anydeals-agent}/bin/anydeals || \
            (echo "FAIL: ANYDEALS_NODE not set in wrapper"; exit 1)
          echo "PASS: ANYDEALS_NODE present in wrapper"

          ANYDEALS_NODE=$(sed -n "s/^export ANYDEALS_NODE='\(.*\)'/\1/p" ${anydeals-agent}/bin/anydeals)
          test -x "$ANYDEALS_NODE" || (echo "FAIL: ANYDEALS_NODE=$ANYDEALS_NODE not executable"; exit 1)
          echo "PASS: ANYDEALS_NODE executable at $ANYDEALS_NODE"

          NODE_MAJOR=$("$ANYDEALS_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ANYDEALS_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ANYDEALS_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "anydeals-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ANYDEALS_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ANYDEALS_MANAGED guards ==="
          check_blocked "config set" ${anydeals-agent}/bin/anydeals config set model foo
          check_blocked "config edit" ${anydeals-agent}/bin/anydeals config edit
        ...
        extra-python-packages = let
          ...
          anydealsWithExtra = anydeals-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "anydeals-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${anydealsWithExtra}/bin/anydeals || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${anydealsWithExtra}/bin/anydeals || \
          ...
          if grep -q "PYTHONPATH" ${anydeals-agent}/bin/anydeals; then
        ...
        extra-dependency-groups = let
          anydealsWithGroups = anydeals-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "anydeals-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${anydealsWithGroups}"
          echo "venv: ${anydealsWithGroups.anydealsVenv}"
        ...
        in pkgs.runCommand "anydeals-config-roundtrip" {
          ...
          merge_and_load() {
            local anydeals_home="$1"
            export ANYDEALS_HOME="$anydeals_home"
            ${configMergeScript} ${nixSettings} "$anydeals_home/config.yaml"
            ${anydealsVenv}/bin/python3 -c '
import json, sys
