# nix/nixosModules.nix — NixOS module for anydeals-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: anydeals runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ANYDEALS_HOME/.env
# and read by anydeals at startup — no container recreation needed for env changes.
#
# Tool resolution: the anydeals wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.anydeals-agent;
    effectivePackage =
    ...
    anydeals-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-anydeals-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "anydeals-config-attrs";
      description = "AnyDeals YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "anydeals-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "anydeals-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ANYDEALS_DOC_EOF'\n${value}\nANYDEALS_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "anydeals-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/anydeals";
    ...
    # anydeals user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "anydeals-container-entrypoint" ''
      set -eu

      ANYDEALS_UID="''${ANYDEALS_UID:?ANYDEALS_UID must be set}"
      ANYDEALS_GID="''${ANYDEALS_GID:?ANYDEALS_GID must be set}"

      # ── Group: ensure a group with GID=$ANYDEALS_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ANYDEALS_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="anydeals"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ANYDEALS_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ANYDEALS_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ANYDEALS_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ANYDEALS_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="anydeals"
        TARGET_HOME="/home/anydeals"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ANYDEALS_UID" -g "$ANYDEALS_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ANYDEALS_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ANYDEALS_UID:$ANYDEALS_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ANYDEALS_HOME is owned by the target user.
      ...
      if [ -n "''${ANYDEALS_HOME:-}" ] && [ -d "$ANYDEALS_HOME" ]; then
        find "$ANYDEALS_HOME" \! -user "$ANYDEALS_UID" -exec chown "$ANYDEALS_UID:$ANYDEALS_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/anydeals-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/anydeals-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/anydeals ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/anydeals
        chmod 0440 /etc/sudoers.d/anydeals
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ANYDEALS_UID" --regid="$ANYDEALS_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ANYDEALS_HOME/.env.
    ...
    # Default: /var/lib/anydeals/workspace → /data/workspace.
  ...
  in {
    options.services.anydeals-agent = with lib; {
      enable = mkEnableOption "AnyDeals Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = anydeals-agent;
        description = "The anydeals-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/anydeals";
        description = "State directory. Contains .anydeals/ subdir (ANYDEALS_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative AnyDeals config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ANYDEALS_HOME/.env at activation time.
          AnyDeals reads this file on every startup via load_anydeals_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ANYDEALS_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ANYDEALS_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `anydeals gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the anydeals user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the anydeals plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. AnyDeals discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as anydeals (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in anydeals-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the anydeals CLI to environment.systemPackages and export
          ANYDEALS_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.anydeals symlink to the service
            stateDir. These users are automatically added to the anydeals group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.anydeals-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the anydeals CLI to system PATH and export ANYDEALS_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.anydeals/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ANYDEALS_HOME = "${cfg.stateDir}/.anydeals";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.anydeals-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the anydeals user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.anydeals-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed anydeals
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure anydeals is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.anydeals        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.anydeals/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.anydeals/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.anydeals/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.anydeals/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.anydeals/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."anydeals-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.anydeals
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.anydeals ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.anydeals ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.anydeals -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.anydeals/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.anydeals/$_subdir"
            chmod 2770 "${cfg.stateDir}/.anydeals/$_subdir"
            find "${cfg.stateDir}/.anydeals/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.anydeals/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.anydeals/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.anydeals/config.yaml
            chmod 0640 ${cfg.stateDir}/.anydeals/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.anydeals/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.anydeals/.managed
          chmod 0644 ${cfg.stateDir}/.anydeals/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.anydeals/.container-mode <<'ANYDEALS_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    anydeals_bin=${containerDataDir}/current-package/bin/anydeals
    ANYDEALS_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.anydeals/.container-mode
            chmod 0644 ${cfg.stateDir}/.anydeals/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.anydeals/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.anydeals";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.anydeals" ]; then
                  rm -f "${symlinkPath}"
                  echo "anydeals-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.anydeals -> stateDir/.anydeals for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.anydeals";
                target = "${cfg.stateDir}/.anydeals";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "anydeals-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.anydeals/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.anydeals/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.anydeals/auth.json
              fi
            ''}
          ''}
          ...
          # AnyDeals reads $ANYDEALS_HOME/.env at startup via load_anydeals_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.anydeals/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ANYDEALS_NIX_ENV_EOF'
    ${envFileContent}
    ANYDEALS_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
        ...
        find ${cfg.stateDir}/.anydeals/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.anydeals/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.anydeals/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.anydeals-agent = {
          description = "AnyDeals Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            ANYDEALS_HOME = "${cfg.stateDir}/.anydeals";
            ANYDEALS_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $ANYDEALS_HOME/.env by the activation script. load_anydeals_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/anydeals"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the anydeals group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.anydeals-agent = {
          description = "AnyDeals Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ANYDEALS_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ANYDEALS_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ANYDEALS_UID="$ANYDEALS_UID" \
                --env ANYDEALS_GID="$ANYDEALS_GID" \
                --env ANYDEALS_HOME=${containerDataDir}/.anydeals \
                --env ANYDEALS_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/anydeals gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
