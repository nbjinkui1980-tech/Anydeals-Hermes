...
## Client ID
- Need to register a "Desktop app" OAuth client on a JINKUI GCP project
...
## Token Lifecycle
- Store at `~/.AnyDeals/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `AnyDeals_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `AnyDeals_cli/models.py` — Add Gemini model catalog
4. `AnyDeals_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `AnyDeals_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `AnyDeals_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- JINKUI GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via ANYDEALS_GEMINI_CLIENT_ID env var
...
- AnyDeals-agent Copilot OAuth: `AnyDeals_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
