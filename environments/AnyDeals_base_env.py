"""
AnydealsAgentBaseEnv -- Abstract Base Environment for Anydeals-Agent + Atropos

Provides the Atropos integration plumbing that all AnyDeals-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via AnydealsAgentLoop
...
# Ensure the AnyDeals-agent repo root is on sys.path so that imports like
...
# Load API keys from AnyDeals-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, AnydealsAgentLoop
...
# Import AnyDeals-agent toolset infrastructure
...
class AnydealsAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for AnyDeals-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of AnyDeals toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: AnyDeals, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/AnyDeals-results/ "
...
class AnydealsAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for AnyDeals-agent Atropos integration.
    ...
    name: Optional[str] = "AnyDeals-agent"
    env_config_cls = AnydealsAgentEnvConfig

    def __init__(
        self,
        config: AnydealsAgentEnvConfig,
        ...
        # Set terminal environment variables so AnyDeals tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY AnyDeals-agent tool (terminal, file, web,
