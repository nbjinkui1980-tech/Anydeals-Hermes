...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs AnydealsAgentLoop with terminal tool only
...
Requires: pip install AnyDeals-agent[yc-bench]
"""
...
class YCBenchEvalConfig(AnydealsAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends AnydealsAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(AnydealsAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The AnydealsAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs AnydealsAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our AnydealsAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the AnydealsAgentLoop
            # ----------------------------------------------------------
