"""
Anydeals MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Anydeals-specific extra)

Usage:
    AnyDeals mcp serve
    AnyDeals mcp serve --verbose
...
logger = logging.getLogger("AnyDeals.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ANYDEALS_HOME."""
    try:
        from AnyDeals_constants import get_AnyDeals_home
        return get_AnyDeals_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ANYDEALS_HOME", Path.home() / ".AnyDeals")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from AnyDeals_constants import get_AnyDeals_home
        directory_file = get_AnyDeals_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ANYDEALS_HOME", Path.home() / ".AnyDeals")
...
class EventBridge:
    ...
    This is the Anydeals equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from AnyDeals_constants import get_AnyDeals_home
            db_file = get_AnyDeals_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ANYDEALS_HOME", Path.home() / ".AnyDeals")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Anydeals MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "AnyDeals",
        instructions=(
            "Anydeals Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Anydeals MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
