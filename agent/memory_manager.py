...
class MemoryManager:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``AnyDeals_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_AnyDeals_home()`` themselves.
        """
        if "AnyDeals_home" not in kwargs:
            from AnyDeals_constants import get_AnyDeals_home
            kwargs["AnyDeals_home"] = str(get_AnyDeals_home())
        for provider in self._providers:
            try:
