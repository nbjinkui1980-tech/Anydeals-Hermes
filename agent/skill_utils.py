...
from AnyDeals_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ANYDEALS_PLATFORM`` or
            ``ANYDEALS_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("ANYDEALS_PLATFORM")
...
# which becomes the dominant cost of ``AnyDeals`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.AnyDeals/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``AnyDeals`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ANYDEALS_HOME, not cwd
        if not p.is_absolute():
            p = (AnyDeals_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.AnyDeals/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    AnyDeals = metadata.get("AnyDeals") or {}
    if not isinstance(AnyDeals, dict):
        AnyDeals = {}
    return {
        "fallback_for_toolsets": AnyDeals.get("fallback_for_toolsets", []),
        "requires_toolsets": AnyDeals.get("requires_toolsets", []),
        "fallback_for_tools": AnyDeals.get("fallback_for_tools", []),
        "requires_tools": AnyDeals.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    AnyDeals = metadata.get("AnyDeals")
    if not isinstance(AnyDeals, dict):
        return []
    raw = AnyDeals.get("config")
    if not raw:
        return []
