"""Anthropic Messages API adapter for Anydeals Agent.

Translates between Anydeals's internal OpenAI-style message format and
...
# Anydeals effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See AnyDeals-agent#13848, #17057.
    """
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Anydeals historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Anydeals)
    ...
    # 1. Anydeals-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Anydeals configs.
...
# ── Anydeals-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.AnyDeals/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_ANYDEALS_OAUTH_FILE = get_AnyDeals_home() / ".anthropic_oauth.json"
...
def run_AnyDeals_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Anydeals-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Anydeals with your Claude Pro/Max subscription.")
    print()
...
def read_AnyDeals_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Anydeals-managed OAuth credentials from ~/.AnyDeals/.anthropic_oauth.json."""
    if _ANYDEALS_OAUTH_FILE.exists():
        try:
            data = json.loads(_ANYDEALS_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Anydeals OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See AnyDeals-agent#13848.
            #
    ...
    # are preserved.  See AnyDeals-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Anydeals Agent", "Claude Code")
                text = text.replace("Anydeals agent", "Claude Code")
                text = text.replace("AnyDeals-agent", "claude-code")
                text = text.replace("JINKUI", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Anydeals surfaces in its CLI. We
