...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.AnyDeals/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Anydeals ↔ models.dev
# ---------------------------------------------------------------------------

# Anydeals provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Anydeals (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_AnyDeals_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``AnyDeals config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Anydeals provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Anydeals agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Anydeals provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Anydeals ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Anydeals or models.dev provider ID.  Tries exact match then
