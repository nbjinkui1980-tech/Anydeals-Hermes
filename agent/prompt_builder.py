...
from AnyDeals_constants import get_AnyDeals_home, get_skills_dir, is_wsl
...
_ANYDEALS_MD_NAMES = (".AnyDeals.md", "ANYDEALS.md")


def _find_AnyDeals_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.AnyDeals.md`` or ``ANYDEALS.md``.
    ...
    for directory in [current, *current.parents]:
        for name in _ANYDEALS_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Anydeals Agent, an intelligent AI assistant created by JINKUI. "
...
ANYDEALS_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Anydeals Agent "
    "itself, load the `AnyDeals-agent` skill with skill_view(name='AnyDeals-agent') "
    "before answering. Docs: https://AnyDeals-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.AnyDeals/kanban.db`. Your task id is in "
    "`$ANYDEALS_KANBAN_TASK`; your workspace is `$ANYDEALS_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ANYDEALS_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `AnyDeals kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Anydeals WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Anydeals itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Anydeals runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Anydeals itself is running. The host OS, home, and cwd "
                f"of the Anydeals process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Anydeals "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_AnyDeals_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.AnyDeals/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ANYDEALS_PLATFORM")
        or get_session_env("ANYDEALS_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Anydeals Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `AnyDeals-agent` skill "
            "first. It has the actual commands (e.g. `AnyDeals config set …`, `AnyDeals tools`, "
            "`AnyDeals setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: AnyDeals setup, AnyDeals setup tools, AnyDeals setup terminal, AnyDeals status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ANYDEALS_HOME and return its content, or None.
    ...
    """
    try:
        from AnyDeals_cli.config import ensure_AnyDeals_home
        ensure_AnyDeals_home()
    except Exception as e:
        logger.debug("Could not ensure ANYDEALS_HOME before loading SOUL.md: %s", e)

    soul_path = get_AnyDeals_home() / "SOUL.md"
...
def _load_AnyDeals_md(cwd_path: Path) -> str:
    """.AnyDeals.md / ANYDEALS.md — walk to git root."""
    AnyDeals_md_path = _find_AnyDeals_md(cwd_path)
    if not AnyDeals_md_path:
        return ""
    try:
        content = AnyDeals_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = AnyDeals_md_path.name
        try:
            rel = str(AnyDeals_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".AnyDeals.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", AnyDeals_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .AnyDeals.md / ANYDEALS.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ANYDEALS_HOME is independent and always included when present.
    ...
    # SOUL.md from ANYDEALS_HOME only — skip when already loaded as identity
    if not skip_soul:
