...
as ``kind: backend``) or ``~/.AnyDeals/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``AnyDeals tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``AnyDeals tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``AnyDeals tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$ANYDEALS_HOME/cache/images/``, creating parents as needed."""
    from AnyDeals_constants import get_AnyDeals_home

    path = get_AnyDeals_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ANYDEALS_HOME/cache/images/``.
