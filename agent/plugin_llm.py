...
Plugins built on Anydeals Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Anydeals
...
:class:`~AnyDeals_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Anydeals' main config
...
handles every provider, fallback chain, and per-task override Anydeals
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`AnyDeals_cli.plugins.PluginContext`
