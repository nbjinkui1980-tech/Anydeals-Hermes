...
Every Anydeals request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Anydeals Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=AnyDeals-agent",
        "client=AnyDeals-client-v<__version__>",
    ]

The version is sourced live from ``AnyDeals_cli.__version__`` so it auto-aligns
...
``AnyDeals_cli.__version__`` is the canonical source of truth.
"""
...
def _AnyDeals_version() -> str:
    """Return the current Anydeals release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``AnyDeals_cli`` cannot be imported (should
...
def AnyDeals_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=AnyDeals-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=AnyDeals-client-v{_AnyDeals_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=AnyDeals-agent", AnyDeals_client_tag()]
