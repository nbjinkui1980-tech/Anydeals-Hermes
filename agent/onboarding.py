...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Anydeals starts and finds ``~/.openclaw/``.

    Points users at ``AnyDeals claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``AnyDeals claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Anydeals, run "
        "`AnyDeals claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `AnyDeals claw cleanup` (renames it to ~/.openclaw.pre-migration — "
