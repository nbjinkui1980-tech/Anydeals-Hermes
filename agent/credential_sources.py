"""Unified removal contract for every credential source Anydeals reads from.

Anydeals seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.AnyDeals/.env
    claude_code   — ~/.claude/.credentials.json
    AnyDeals_pkce   — ~/.AnyDeals/.anthropic_oauth.json
    ...
    manual        — user ran `AnyDeals auth add`
...
unify here is **removal**:
    ...
    ``AnyDeals auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), AnyDeals_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.AnyDeals/.env  → clear it
    ...
    """
    from AnyDeals_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.AnyDeals/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Anydeals.",
            f"  The pool entry is now suppressed — Anydeals will ignore "
            f"{env_var} until you run `AnyDeals auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Anydeals stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `AnyDeals auth add anthropic` to re-enable if needed.",
    ])


def _remove_AnyDeals_pkce(provider: str, removed) -> RemovalResult:
    """~/.AnyDeals/.anthropic_oauth.json is ours — delete it outright."""
    from AnyDeals_constants import get_AnyDeals_home

    result = RemovalResult()
    oauth_file = get_AnyDeals_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Anydeals Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `AnyDeals login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `AnyDeals auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Anydeals auth store is not enough — _seed_from_singletons() would
    ...
    via ``AnyDeals auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from AnyDeals_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `AnyDeals auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `AnyDeals auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Anydeals stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `AnyDeals auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="AnyDeals_pkce",
        remove_fn=_remove_AnyDeals_pkce,
        description="~/.AnyDeals/.anthropic_oauth.json",
    ))
    register(RemovalStep(
