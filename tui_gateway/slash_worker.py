"""Persistent slash-command worker — one AnydealsCLI per TUI session.
...
def _run(cli: AnydealsCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ANYDEALS_SESSION_KEY"] = args.session_key
    os.environ["ANYDEALS_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AnydealsCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
