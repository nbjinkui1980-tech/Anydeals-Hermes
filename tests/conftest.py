"""Shared fixtures for the AnyDeals-agent test suite.
...
2. **Isolated ANYDEALS_HOME.** ANYDEALS_HOME points to a per-test tempdir so
   code reading ``~/.AnyDeals/*`` via ``get_AnyDeals_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".AnyDeals"`` instead of the canonical
   ``get_AnyDeals_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ANYDEALS_SESSION_* inheritance** — the agent's current gateway
...
# ANYDEALS_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ANYDEALS_BEHAVIORAL_VARS = frozenset({
    "ANYDEALS_YOLO_MODE",
    "ANYDEALS_INTERACTIVE",
    "ANYDEALS_QUIET",
    "ANYDEALS_TOOL_PROGRESS",
    "ANYDEALS_TOOL_PROGRESS_MODE",
    "ANYDEALS_MAX_ITERATIONS",
    "ANYDEALS_SESSION_PLATFORM",
    "ANYDEALS_SESSION_CHAT_ID",
    "ANYDEALS_SESSION_CHAT_NAME",
    "ANYDEALS_SESSION_THREAD_ID",
    "ANYDEALS_SESSION_SOURCE",
    "ANYDEALS_SESSION_KEY",
    "ANYDEALS_GATEWAY_SESSION",
    "ANYDEALS_PLATFORM",
    "ANYDEALS_MODEL",
    "ANYDEALS_INFERENCE_MODEL",
    "ANYDEALS_INFERENCE_PROVIDER",
    "ANYDEALS_TUI_PROVIDER",
    "ANYDEALS_MANAGED",
    "ANYDEALS_DEV",
    "ANYDEALS_CONTAINER",
    "ANYDEALS_EPHEMERAL_SYSTEM_PROMPT",
    "ANYDEALS_TIMEZONE",
    "ANYDEALS_REDACT_SECRETS",
    "ANYDEALS_BACKGROUND_NOTIFICATIONS",
    "ANYDEALS_EXEC_ASK",
    "ANYDEALS_HOME_MODE",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.AnyDeals/kanban.db instead of the per-test ANYDEALS_HOME.
    "ANYDEALS_KANBAN_DB",
    "ANYDEALS_KANBAN_BOARD",
    "ANYDEALS_KANBAN_WORKSPACES_ROOT",
    "ANYDEALS_KANBAN_LOGS_ROOT",
    "ANYDEALS_KANBAN_TASK",
    "ANYDEALS_KANBAN_WORKSPACE",
    "ANYDEALS_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ANYDEALS_HOME to per-test tempdirs so code that
    reads ``~/.AnyDeals/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ANYDEALS_* vars that could change test semantics.
    for name in _ANYDEALS_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ANYDEALS_HOME to a per-test tempdir. Code that reads
    #    ``~/.AnyDeals/*`` via ``get_AnyDeals_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.AnyDeals/*`` via
    #    ``Path.home() / ".AnyDeals"`` instead of ``get_AnyDeals_home()``
    #    is a bug to fix at the callsite.
    fake_AnyDeals_home = tmp_path / "AnyDeals_test"
    fake_AnyDeals_home.mkdir()
    (fake_AnyDeals_home / "sessions").mkdir()
    (fake_AnyDeals_home / "cron").mkdir()
    (fake_AnyDeals_home / "memories").mkdir()
    (fake_AnyDeals_home / "skills").mkdir()
    monkeypatch.setenv("ANYDEALS_HOME", str(fake_AnyDeals_home))
    ...
    #    ~/.AnyDeals/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import AnyDeals_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_AnyDeals_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "AnyDeals_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal AnyDeals config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``AnyDeals-gateway`` process
...
# correlated with a ``tests/AnyDeals_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> AnyDeals-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart AnyDeals-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting AnyDeals/python patterns are also blocked.
    """
    ...
    def _matches_AnyDeals_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ANYDEALS_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_AnyDeals_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch AnyDeals-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m AnyDeals_cli.main".
                if (
                    "AnyDeals" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live AnyDeals-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting AnyDeals/python could hit the live gateway. "
