"""Tests for AnyDeals_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Anydeals entry point
(AnyDeals, AnyDeals-agent, AnyDeals-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Anydeals entry point imports AnyDeals_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported AnyDeals_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("AnyDeals_bootstrap", None)
    import AnyDeals_bootstrap  # noqa: WPS433
    return AnyDeals_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Anydeals CLI
...
class TestEntryPointsImportBootstrap:
    """Every Anydeals entry point must import AnyDeals_bootstrap as its
    ...
    # Entry points that invoke Anydeals as a process.  Each one must
    # import AnyDeals_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "AnyDeals_cli/main.py",   # AnyDeals CLI (console_script)
        "run_agent.py",          # AnyDeals-agent (console_script)
        "acp_adapter/entry.py",  # AnyDeals-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import AnyDeals_bootstrap' and that
        ...
        half-finished ``AnyDeals update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``AnyDeals_bootstrap`` as a top-level module) leaves AnyDeals
        ...
        """
        # Resolve relative to the AnyDeals-agent repo root.  Tests live
        # at tests/test_AnyDeals_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # AnyDeals start even when AnyDeals_bootstrap hasn't been
        ...
        assert first_import_name == "AnyDeals_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'AnyDeals_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import AnyDeals_bootstrap' line to be the first import."
        )
