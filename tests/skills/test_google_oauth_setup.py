...
class TestAnydealsConstantsFallback:
    """Tests for _AnyDeals_home.py fallback when AnyDeals_constants is unavailable."""
    ...
    def _load_helper(self, monkeypatch):
        """Load _AnyDeals_home.py with AnyDeals_constants blocked."""
        monkeypatch.setitem(sys.modules, "AnyDeals_constants", None)
        spec = importlib.util.spec_from_file_location("_AnyDeals_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_AnyDeals_home_env_var(self, monkeypatch, tmp_path):
        """When AnyDeals_constants is missing, ANYDEALS_HOME comes from env var."""
        monkeypatch.setenv("ANYDEALS_HOME", str(tmp_path / "custom-AnyDeals"))
        module = self._load_helper(monkeypatch)
        assert module.get_AnyDeals_home() == tmp_path / "custom-AnyDeals"

    def test_fallback_defaults_to_dot_AnyDeals(self, monkeypatch):
        """When AnyDeals_constants is missing and ANYDEALS_HOME unset, default to ~/.AnyDeals."""
        monkeypatch.delenv("ANYDEALS_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_AnyDeals_home() == Path.home() / ".AnyDeals"

    def test_fallback_ignores_empty_AnyDeals_home(self, monkeypatch):
        """Empty/whitespace ANYDEALS_HOME is treated as unset."""
        monkeypatch.setenv("ANYDEALS_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_AnyDeals_home() == Path.home() / ".AnyDeals"

    def test_fallback_display_AnyDeals_home_shortens_path(self, monkeypatch):
        """Fallback display_AnyDeals_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ANYDEALS_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_AnyDeals_home() == "~/.AnyDeals"

    def test_fallback_display_AnyDeals_home_profile_path(self, monkeypatch):
        """Fallback display_AnyDeals_home() handles profile paths under ~/."""
        monkeypatch.setenv("ANYDEALS_HOME", str(Path.home() / ".AnyDeals/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_AnyDeals_home() == "~/.AnyDeals/profiles/coder"

    def test_fallback_display_AnyDeals_home_custom_path(self, monkeypatch):
        """Fallback display_AnyDeals_home() returns full path for non-home locations."""
        monkeypatch.setenv("ANYDEALS_HOME", "/opt/AnyDeals-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_AnyDeals_home() == "/opt/AnyDeals-custom"

    def test_delegates_to_AnyDeals_constants_when_available(self):
        """When AnyDeals_constants IS importable, _AnyDeals_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_AnyDeals_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import AnyDeals_constants
        assert module.get_AnyDeals_home is AnyDeals_constants.get_AnyDeals_home
        assert module.display_AnyDeals_home is AnyDeals_constants.display_AnyDeals_home
