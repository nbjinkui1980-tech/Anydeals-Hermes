...
def server():
    with patch.dict("sys.modules", {
        "AnyDeals_constants": MagicMock(get_AnyDeals_home=MagicMock(return_value="/tmp/AnyDeals_test")),
        "AnyDeals_cli.env_loader": MagicMock(),
        "AnyDeals_cli.banner": MagicMock(),
        "AnyDeals_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `ANYDEALS_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("ANYDEALS_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("ANYDEALS_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_config_load_missing(server, tmp_path):
    server._AnyDeals_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/AnyDeals-agent-dev": {"name": "AnyDeals-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "AnyDeals-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "AnyDeals_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "AnyDeals-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "AnyDeals-agent-dev"
