...
def server():
    with patch.dict(
        "sys.modules",
        {
            "AnyDeals_constants": MagicMock(
                get_AnyDeals_home=MagicMock(return_value="/tmp/AnyDeals_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'AnyDeals-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'AnyDeals-release' patched"
    }
