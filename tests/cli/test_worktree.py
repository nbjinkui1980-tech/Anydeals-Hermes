...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("AnyDeals/AnyDeals-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned AnyDeals/* and pr-* branches."""

    def test_prunes_orphaned_AnyDeals_branch(self, git_repo):
        """AnyDeals/AnyDeals-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "AnyDeals/AnyDeals-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "AnyDeals/AnyDeals-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "AnyDeals/AnyDeals-deadbeef" in result.stdout
        ...
        assert "AnyDeals/AnyDeals-deadbeef" in orphaned
        ...
        assert "AnyDeals/AnyDeals-deadbeef" not in result.stdout
