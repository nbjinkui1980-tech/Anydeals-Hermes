...
def _isolate(tmp_path, monkeypatch):
    """Isolate ANYDEALS_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal AnydealsCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = AnydealsCLI.__new__(AnydealsCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["AnyDeals-core"]
        obj.compact = False
