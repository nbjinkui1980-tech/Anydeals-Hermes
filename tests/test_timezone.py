"""
Tests for timezone support (AnyDeals_time module + integration points).
...
import AnyDeals_time


def _reset_AnyDeals_time_cache():
    """Reset the AnyDeals_time module cache (replacement for removed reset_cache)."""
    AnyDeals_time._cached_tz = None
    AnyDeals_time._cached_tz_name = None
    AnyDeals_time._cache_resolved = False


# =========================================================================
# AnyDeals_time.now() — core helper
# =========================================================================

class TestAnydealsTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_AnyDeals_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["ANYDEALS_TIMEZONE"] = "Asia/Kolkata"
        result = AnyDeals_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["ANYDEALS_TIMEZONE"] = "UTC"
        result = AnyDeals_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["ANYDEALS_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="AnyDeals_time"):
            result = AnyDeals_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["ANYDEALS_TIMEZONE"] = "UTC"
        _reset_AnyDeals_time_cache()
        r1 = AnyDeals_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["ANYDEALS_TIMEZONE"] = "Asia/Kolkata"
        _reset_AnyDeals_time_cache()
        r2 = AnyDeals_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ANYDEALS_TIMEZONE"] = "Europe/London"
        tz = AnyDeals_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["ANYDEALS_TIMEZONE"] = "Not/A/Timezone"
        tz = AnyDeals_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ANYDEALS_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ANYDEALS_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ANYDEALS_TIMEZONE=" + os.environ.get("ANYDEALS_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ANYDEALS_TIMEZONE=NOT_SET" in result["output"], (
            "ANYDEALS_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ANYDEALS_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ANYDEALS_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=AnyDeals_tz) which shifted
        absolute time when system-local tz != Anydeals tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_AnyDeals_tz(self):
        """Already-aware datetimes should be normalized to Anydeals tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Anydeals tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Anydeals tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Anydeals is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Anydeals tz
        ...
        # Use a Anydeals timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _AnyDeals_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ANYDEALS_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_AnyDeals_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Anydeals timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ANYDEALS_TIMEZONE"] = "US/Eastern"
        _reset_AnyDeals_time_cache()
