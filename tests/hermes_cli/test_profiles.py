"""Comprehensive tests for AnyDeals_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and ANYDEALS_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.AnyDeals/profiles)
    * ANYDEALS_HOME  -> tmp_path/.AnyDeals  (so get_AnyDeals_home() agrees)
    * Creates the bare-minimum ~/.AnyDeals directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["AnyDeals", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Anydeals install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_AnyDeals_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".AnyDeals"

    def test_named_profile_returns_profiles_subdir(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("coder")
        assert result == tmp_path / ".AnyDeals" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".AnyDeals" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.AnyDeals must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".AnyDeals"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes AnyDeals-agent, .worktrees,
        ...
        (default_home / "AnyDeals-agent" / ".git").mkdir(parents=True)
        (default_home / "AnyDeals-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "AnyDeals-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "AnyDeals-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `AnyDeals profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `AnyDeals update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("AnyDeals_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".AnyDeals" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_AnyDeals_home_returns_default(self, profile_env):
        # ANYDEALS_HOME points to tmp_path/.AnyDeals which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".AnyDeals" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom ANYDEALS_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("ANYDEALS_HOME", str(custom))
        # With Docker-aware roots, a custom ANYDEALS_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("AnyDeals")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".AnyDeals" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("AnyDeals_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".AnyDeals" / "profiles" / "newname"

    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        tmp_path = profile_env
        create_profile("ssi_health", no_alias=True)
        honcho_path = tmp_path / ".AnyDeals" / "honcho.json"
        ...
        assert "AnyDeals.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["AnyDeals.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["AnyDeals.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "AnyDeals.ssi_health": {"workspace": "AnyDeals", "enabled": True}
            }
        }))
        ...
        assert "AnyDeals.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["AnyDeals.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["AnyDeals.heimdall"]["workspace"] == "AnyDeals"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "AnyDeals.ssi_health": {"aiPeer": "ssi_health"},
                "AnyDeals.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["AnyDeals.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["AnyDeals.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("AnyDeals-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".AnyDeals_history",
        ...
        excluded_prefixes = [
            "default/AnyDeals-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.AnyDeals_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_AnyDeals_profiles_function(self):
        script = generate_bash_completion()
        assert "_AnyDeals_profiles" in script

    def test_zsh_completion_has_AnyDeals_function(self):
        script = generate_zsh_completion()
        assert "_AnyDeals" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultAnydealsHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_AnyDeals_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".AnyDeals" / "profiles"

    def test_default_AnyDeals_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_AnyDeals_home()
        assert home == tmp_path / ".AnyDeals"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (ANYDEALS_HOME outside ~/.AnyDeals), profiles go under ANYDEALS_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANYDEALS_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_AnyDeals_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_AnyDeals_home() returns ANYDEALS_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANYDEALS_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (ANYDEALS_HOME under ~/.AnyDeals), profiles root is still ~/.AnyDeals/profiles."""
        native = tmp_path / ".AnyDeals"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under ANYDEALS_HOME."""
        from AnyDeals_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANYDEALS_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under ANYDEALS_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANYDEALS_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANYDEALS_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".AnyDeals" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from AnyDeals_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".AnyDeals"
