...
The notice prints the most recent curator run summary on `AnyDeals update`,
...
session start) so users normally never see the rename map. `AnyDeals update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(AnyDeals_main)
