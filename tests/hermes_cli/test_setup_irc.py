"""Tests for IRC gateway configuration via `AnyDeals setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Anydeals install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Anydeals install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#AnyDeals")
            monkeypatch.setenv("IRC_NICKNAME", "AnyDeals-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `AnyDeals setup gateway` experience with IRC present."""
