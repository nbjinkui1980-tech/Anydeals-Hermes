...
""",
        ...
        from AnyDeals_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        (AnyDeals_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ANYDEALS_HOME", str(AnyDeals_home))
        from AnyDeals_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from AnyDeals_cli.plugins import PluginContext, PluginManifest, PluginManager
