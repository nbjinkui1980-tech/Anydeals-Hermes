...
def test_AnyDeals_client_tag_includes_current_version():
    """The client tag must reflect AnyDeals_cli.__version__ verbatim."""
    from AnyDeals_cli import __version__
    from agent.portal_tags import AnyDeals_client_tag

    assert AnyDeals_client_tag() == f"client=AnyDeals-client-v{__version__}"


def test_AnyDeals_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import AnyDeals_client_tag

    tag = AnyDeals_client_tag()
    assert tag.startswith("client=AnyDeals-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import AnyDeals_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=AnyDeals-agent" in tags
    assert AnyDeals_client_tag() in tags
    assert len(tags) == 2
