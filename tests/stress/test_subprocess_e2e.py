...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves AnyDeals_cli.kanban_db on its own
  ...
  - worker logs are captured to ANYDEALS_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "ANYDEALS_KANBAN_TASK": task.id,
            "ANYDEALS_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="AnyDeals_e2e_")
    os.environ["ANYDEALS_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from AnyDeals_cli import kanban_db as kb

    # Point the `AnyDeals` CLI child processes will run at the worktree
    # AnyDeals_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "AnyDeals")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m AnyDeals_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/AnyDeals_e2e_*/worker_*.log
    import glob
