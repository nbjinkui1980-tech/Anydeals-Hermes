"""Tests for the top-level `./AnyDeals` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./AnyDeals` should use `AnyDeals_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "AnyDeals"
    called = []

    fake_main_module = types.ModuleType("AnyDeals_cli.main")

    def fake_main():
        called.append("AnyDeals_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "AnyDeals_cli.main", fake_main_module)
    ...
    assert called == ["AnyDeals_cli.main"]
