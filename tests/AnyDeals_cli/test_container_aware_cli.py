...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ANYDEALS_HOME with .container-mode file."""
    AnyDeals_home = tmp_path / ".AnyDeals"
    AnyDeals_home.mkdir()
    monkeypatch.setenv("ANYDEALS_HOME", str(AnyDeals_home))
    monkeypatch.delenv("ANYDEALS_DEV", raising=False)

    container_mode = AnyDeals_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=AnyDeals-agent\n"
        "exec_user=AnyDeals\n"
        "AnyDeals_bin=/data/current-package/bin/AnyDeals\n"
    )
    return AnyDeals_home


def test_get_container_exec_info_returns_metadata(container_env):
    """Reads .container-mode and returns all fields including exec_user."""
    with patch("AnyDeals_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "AnyDeals-agent"
    assert info["exec_user"] == "AnyDeals"
    assert info["AnyDeals_bin"] == "/data/current-package/bin/AnyDeals"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("AnyDeals_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_AnyDeals_dev(container_env, monkeypatch):
    """Returns None when ANYDEALS_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ANYDEALS_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_AnyDeals_dev_zero(container_env, monkeypatch):
    """ANYDEALS_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ANYDEALS_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        AnyDeals_home = Path(tmpdir) / ".AnyDeals"
        AnyDeals_home.mkdir()
        (AnyDeals_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("AnyDeals_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_AnyDeals_home": lambda: AnyDeals_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ANYDEALS_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=AnyDeals-custom\n"
        "exec_user=myuser\n"
        "AnyDeals_bin=/opt/AnyDeals/bin/AnyDeals\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "AnyDeals-custom"
    assert info["exec_user"] == "myuser"
    assert info["AnyDeals_bin"] == "/opt/AnyDeals/bin/AnyDeals"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from AnyDeals_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "AnyDeals"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "AnyDeals-agent" in cmd
    assert "/data/current-package/bin/AnyDeals" in cmd
    assert "chat" in cmd
