...
See: https://github.com/NousResearch/AnyDeals-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for AnyDeals_constants.get_subprocess_home()."""

    def test_returns_none_when_AnyDeals_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        profile_home = AnyDeals_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("ANYDEALS_HOME", str(AnyDeals_home))
        from AnyDeals_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".AnyDeals" / "profiles"
        ...
        monkeypatch.setenv("ANYDEALS_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("ANYDEALS_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(AnyDeals_home / "home")
    ...
    def test_no_injection_when_AnyDeals_home_unset(self, monkeypatch):
        monkeypatch.delenv("ANYDEALS_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from AnyDeals_cli.profiles import _PROFILE_DIRS
