...
class TestToolsetConsistency:
    ...
    def test_AnyDeals_platforms_share_core_tools(self):
        """All AnyDeals-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on AnyDeals-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["AnyDeals-cli", "AnyDeals-telegram", "AnyDeals-discord", "AnyDeals-whatsapp", "AnyDeals-slack", "AnyDeals-signal", "AnyDeals-homeassistant"]
