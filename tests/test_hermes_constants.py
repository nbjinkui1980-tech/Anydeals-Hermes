"""Tests for AnyDeals_constants module."""
...
from AnyDeals_constants import (
    ...
    get_default_AnyDeals_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultAnydealsRoot:
    """Tests for get_default_AnyDeals_root() — Docker/custom deployment awareness."""

    def test_no_AnyDeals_home_returns_native(self, tmp_path, monkeypatch):
        """When ANYDEALS_HOME is not set, returns ~/.AnyDeals."""
        monkeypatch.delenv("ANYDEALS_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_AnyDeals_root() == tmp_path / ".AnyDeals"

    def test_AnyDeals_home_is_native(self, tmp_path, monkeypatch):
        """When ANYDEALS_HOME = ~/.AnyDeals, returns ~/.AnyDeals."""
        native = tmp_path / ".AnyDeals"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANYDEALS_HOME", str(native))
        assert get_default_AnyDeals_root() == native

    def test_AnyDeals_home_is_profile(self, tmp_path, monkeypatch):
        """When ANYDEALS_HOME is a profile under ~/.AnyDeals, returns ~/.AnyDeals."""
        native = tmp_path / ".AnyDeals"
    ...
    def test_AnyDeals_home_is_docker(self, tmp_path, monkeypatch):
        """When ANYDEALS_HOME points outside ~/.AnyDeals (Docker), returns ANYDEALS_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANYDEALS_HOME", str(docker_home))
        assert get_default_AnyDeals_root() == docker_home

    def test_AnyDeals_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ANYDEALS_HOME outside ~/.AnyDeals is treated as the root."""
        custom = tmp_path / "my-AnyDeals-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANYDEALS_HOME", str(custom))
        assert get_default_AnyDeals_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ANYDEALS_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANYDEALS_HOME", str(profile))
        assert get_default_AnyDeals_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(AnyDeals_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(AnyDeals_constants, "_container_detected", True)
        assert is_container() is True
