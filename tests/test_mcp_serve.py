"""
Tests for mcp_serve — Anydeals MCP server.
...
def _isolate_AnyDeals_home(tmp_path, monkeypatch):
    """Redirect ANYDEALS_HOME to a temp directory."""
    monkeypatch.setenv("ANYDEALS_HOME", str(tmp_path))
    try:
        import AnyDeals_constants
        monkeypatch.setattr(AnyDeals_constants, "get_AnyDeals_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking AnyDeals_state schema."""
