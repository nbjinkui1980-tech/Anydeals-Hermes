...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.AnyDeals/.update_response."""
        adapter = self._make_adapter()
        AnyDeals_home = tmp_path / "AnyDeals_home"
        AnyDeals_home.mkdir()
        monkeypatch.setattr(
            "AnyDeals_constants.get_AnyDeals_home",
            lambda: AnyDeals_home,
        )
        ...
        response = AnyDeals_home / ".update_response"
