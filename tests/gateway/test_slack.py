...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.AnyDeals."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /AnyDeals, /btw, /stop,
        ...
        for expected in ("/AnyDeals", "/btw", "/stop", "/model", "/help"):
...
class TestSlashCommands:
    ...
    # instead of as /AnyDeals subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /AnyDeals btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_AnyDeals_prefix_still_works(self, adapter):
        """Backward compat: /AnyDeals btw foo must still route to /btw foo.

        Old workspace manifests only declared /AnyDeals as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_AnyDeals_freeform_question(self, adapter):
        """/AnyDeals <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/AnyDeals",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_AnyDeals_slash_stashes_context(self, adapter):
        """Legacy /AnyDeals <subcommand> also stashes context."""
        command = {
            "command": "/AnyDeals",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_AnyDeals_question_does_not_stash_context(self, adapter):
        """Free-form /AnyDeals <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/AnyDeals",
