...
- Path resolution (absolute, relative to ANYDEALS_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ANYDEALS_HOME."""
    AnyDeals_home = tmp_path / ".AnyDeals"
    AnyDeals_home.mkdir()
    (AnyDeals_home / "cron").mkdir()
    (AnyDeals_home / "cron" / "output").mkdir()
    (AnyDeals_home / "scripts").mkdir()
    monkeypatch.setenv("ANYDEALS_HOME", str(AnyDeals_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ANYDEALS_DIR", AnyDeals_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", AnyDeals_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", AnyDeals_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", AnyDeals_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.AnyDeals/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ANYDEALS_SESSION_PLATFORM") is None
        assert os.environ.get("ANYDEALS_SESSION_CHAT_ID") is None
        assert os.environ.get("ANYDEALS_SESSION_CHAT_NAME") is None
