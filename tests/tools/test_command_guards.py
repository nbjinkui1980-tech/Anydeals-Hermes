...
def _clean_state():
    ...
    saved = {}
    for k in ("ANYDEALS_INTERACTIVE", "ANYDEALS_GATEWAY_SESSION", "ANYDEALS_EXEC_ASK", "ANYDEALS_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ANYDEALS_INTERACTIVE"] = "1"
        session_key = os.getenv("ANYDEALS_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ANYDEALS_INTERACTIVE or ANYDEALS_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
