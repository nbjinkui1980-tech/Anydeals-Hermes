...
class TestWriteDenyExactPaths:
    ...
    def test_AnyDeals_env(self):
        # ``.env`` under the active ANYDEALS_HOME (profile-aware, not just
        # ``~/.AnyDeals``) must be write-denied. The hermetic test conftest
        # points ANYDEALS_HOME at a tempdir — resolve via get_AnyDeals_home()
        # to match the denylist.
        from AnyDeals_constants import get_AnyDeals_home
        path = str(get_AnyDeals_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_AnyDeals_config_not_env(self):
        path = os.path.join(str(Path.home()), ".AnyDeals", "config.yaml")
