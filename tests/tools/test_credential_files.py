...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        AnyDeals_home.mkdir()
        (AnyDeals_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(AnyDeals_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.AnyDeals/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        (AnyDeals_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        (AnyDeals_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        (AnyDeals_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        AnyDeals_home = tmp_path / ".AnyDeals"
        skills_dir = AnyDeals_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.AnyDeals/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        AnyDeals_home = tmp_path / ".AnyDeals"
        (AnyDeals_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ANYDEALS_HOME": str(AnyDeals_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.AnyDeals")

        assert mounts[0]["container_path"] == "/home/user/.AnyDeals/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.AnyDeals/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.AnyDeals/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ANYDEALS_HOME."""
        monkeypatch.setenv("ANYDEALS_HOME", str(tmp_path / ".AnyDeals"))
        (tmp_path / ".AnyDeals").mkdir()

        # Create a sensitive file one level above AnyDeals_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside AnyDeals_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ANYDEALS_HOME must still be registered."""
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        monkeypatch.setenv("ANYDEALS_HOME", str(AnyDeals_home))
        (AnyDeals_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_AnyDeals_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ANYDEALS_HOME must be allowed."""
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        subdir = AnyDeals_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ANYDEALS_HOME pointing outside must be rejected."""
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        monkeypatch.setenv("ANYDEALS_HOME", str(AnyDeals_home))

        # Create a sensitive file outside AnyDeals_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside AnyDeals_home pointing outside
        symlink = AnyDeals_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ANYDEALS_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, AnyDeals_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ANYDEALS_HOME."""
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        monkeypatch.setenv("ANYDEALS_HOME", str(AnyDeals_home))
        ...
        sensitive.write_text("{}")
        self._write_config(AnyDeals_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(AnyDeals_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ANYDEALS_HOME via config must still mount."""
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        monkeypatch.setenv("ANYDEALS_HOME", str(AnyDeals_home))

        (AnyDeals_home / "oauth.json").write_text("{}")
        self._write_config(AnyDeals_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        (AnyDeals_home / "cache" / "documents").mkdir(parents=True)
        (AnyDeals_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("ANYDEALS_HOME", str(AnyDeals_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.AnyDeals/cache/documents" in paths
        assert "/root/.AnyDeals/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.AnyDeals/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        AnyDeals_home = tmp_path / ".AnyDeals"
        AnyDeals_home.mkdir()
        # Use legacy dir name — get_AnyDeals_dir prefers old if it exists
        (AnyDeals_home / "document_cache").mkdir()
        (AnyDeals_home / "image_cache").mkdir()
        monkeypatch.setenv("ANYDEALS_HOME", str(AnyDeals_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(AnyDeals_home / "document_cache") in host_paths
        assert str(AnyDeals_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.AnyDeals/cache/documents" in container_paths
        assert "/root/.AnyDeals/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        AnyDeals_home = tmp_path / ".AnyDeals"
        doc_dir = AnyDeals_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        AnyDeals_home = tmp_path / ".AnyDeals"
        ss_dir = AnyDeals_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.AnyDeals/cache/screenshots/session_abc/screen1.png"
