...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_AnyDeals_home_override(self, tmp_path):
        """Auth lookup should read from ANYDEALS_HOME/auth.json, not ~/.AnyDeals/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".AnyDeals").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "ANYDEALS_HOME": str(AnyDeals_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``AnyDeals tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
