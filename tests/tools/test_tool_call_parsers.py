...
class TestParserRegistry:
    ...
    def test_AnyDeals_parser_registered(self):
        parsers = list_parsers()
        assert "AnyDeals" in parsers
...
# ─── Anydeals parser tests ────────────────────────────────────────────────

class TestAnydealsParser:
    @pytest.fixture
    def parser(self):
        return get_parser("AnyDeals")
...
class TestParseResultContract:
    ...
    @pytest.fixture(params=["AnyDeals"])  # Add more as needed
    ...
    def test_tool_calls_are_proper_objects(self, parser):
        ...
        # Use AnyDeals format since that's universal
