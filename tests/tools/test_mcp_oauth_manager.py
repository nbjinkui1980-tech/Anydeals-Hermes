...
def test_AnyDeals_provider_subclass_exists():
    """AnydealsMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ANYDEALS_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ANYDEALS_PROVIDER_CLS is not None
    assert issubclass(_ANYDEALS_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_AnyDeals_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AnydealsMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ANYDEALS_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ANYDEALS_HOME", str(tmp_path))
    ...
    assert _ANYDEALS_PROVIDER_CLS is not None
    assert isinstance(provider, _ANYDEALS_PROVIDER_CLS)
    assert provider._AnyDeals_server_name == "srv"
