...
def _restore_tool_modules():
    original_AnyDeals_home = os.environ.get("ANYDEALS_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "AnyDeals_cli"
        or name.startswith("AnyDeals_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_AnyDeals_home is None:
            os.environ.pop("ANYDEALS_HOME", None)
        else:
            os.environ["ANYDEALS_HOME"] = original_AnyDeals_home
        _reset_modules(("tools", "AnyDeals_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    AnyDeals_cli = types.ModuleType("AnyDeals_cli")
    AnyDeals_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["AnyDeals_cli"] = AnyDeals_cli
    AnyDeals_home = tmp_path / "AnyDeals-home"
    os.environ["ANYDEALS_HOME"] = str(AnyDeals_home)
    sys.modules["AnyDeals_cli.config"] = types.SimpleNamespace(
        get_AnyDeals_home=lambda: AnyDeals_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="AnyDeals-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": AnyDeals_home / "modal_snapshots.json",
