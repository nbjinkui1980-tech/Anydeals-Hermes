...
- Signal forwarding runs through the init so ``docker stop`` triggers
  AnyDeals's own graceful-shutdown path.
"""
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, AnyDeals accumulates zombie
    ...
    assert installed, (
        ...
        "orphaned subprocesses, AnyDeals accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into AnyDeals,
    ...
    assert routes_through_init, (
        ...
        "If tini is only installed but not wired into ENTRYPOINT, AnyDeals "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``AnyDeals-ink`` workspace package is
    ...
    assert "ui-tui/packages/AnyDeals-ink/" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    # ``AnyDeals-ink`` is a bundled workspace package referenced from
    # ``ui-tui/package.json`` via ``file:`` — not pulled from the npm
    # registry. The contract this test pins is just that the image
    # actually carries the package source so ``await import('@AnyDeals/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@AnyDeals/ink``)
    ...
    assert "ui-tui/packages/AnyDeals-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled AnyDeals-ink workspace package "
        "so ``await import('@AnyDeals/ink')`` resolves at runtime."
    )
