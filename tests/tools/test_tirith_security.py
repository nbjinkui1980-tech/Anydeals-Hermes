...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._AnyDeals_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_AnyDeals_bin(self):
        """After _INSTALL_FAILED, manual install in ANYDEALS_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        AnyDeals_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(AnyDeals_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(AnyDeals_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._AnyDeals_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == AnyDeals_bin
            assert _tirith_mod._resolved_path == AnyDeals_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ANYDEALS_HOME isolation
# ---------------------------------------------------------------------------

class TestAnydealsHomeIsolation:
    def test_AnyDeals_bin_dir_respects_AnyDeals_home(self):
        """_AnyDeals_bin_dir must use ANYDEALS_HOME, not hardcoded ~/.AnyDeals."""
        from tools.tirith_security import _AnyDeals_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ANYDEALS_HOME": tmpdir}):
            result = _AnyDeals_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_AnyDeals_home(self):
        """_failure_marker_path must use ANYDEALS_HOME, not hardcoded ~/.AnyDeals."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ANYDEALS_HOME": "/custom/AnyDeals"}):
            result = _failure_marker_path()
        assert result == "/custom/AnyDeals/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ANYDEALS_HOME; verify it's active."""
        AnyDeals_home = os.getenv("ANYDEALS_HOME")
        assert AnyDeals_home is not None, "ANYDEALS_HOME should be set by conftest"
        assert "AnyDeals_test" in AnyDeals_home, "Should point to test temp dir"

    def test_get_AnyDeals_home_fallback(self):
        """Without ANYDEALS_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_AnyDeals_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ANYDEALS_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".AnyDeals")
            result = _get_AnyDeals_home()
