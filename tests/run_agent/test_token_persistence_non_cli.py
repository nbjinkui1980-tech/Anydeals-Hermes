...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    AnyDeals_state = ModuleType("AnyDeals_state")
    AnyDeals_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "AnyDeals_state", AnyDeals_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Anydeals"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Anydeals"
