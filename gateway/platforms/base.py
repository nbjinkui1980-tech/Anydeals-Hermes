...
# Audio file extensions Anydeals recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Anydeals' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Anydeals-created Telegram private-chat
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.AnyDeals/.env manually."
)
...
# Default location: {ANYDEALS_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_AnyDeals_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_AnyDeals_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_AnyDeals_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_AnyDeals_dir("cache/documents", "document_cache")
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?AnyDeals\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+AnyDeals[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ANYDEALS_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ANYDEALS_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ANYDEALS_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ANYDEALS_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("ANYDEALS_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ANYDEALS_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _AnyDeals_run_generation is set on
