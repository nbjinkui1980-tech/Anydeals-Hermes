...
def _get_AnyDeals_version() -> str:
    """Return the AnyDeals-agent package version, or 'dev' if unavailable."""
    try:
        from importlib.metadata import version
        return version("AnyDeals-agent")
    except Exception:
        return "dev"
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Anydeals/<AnyDeals_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; Anydeals/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    AnyDeals_version = _get_AnyDeals_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; Anydeals/{AnyDeals_version})"
