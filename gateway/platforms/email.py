"""
Email platform adapter for the Anydeals gateway.

Allows users to interact with Anydeals by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "AnyDeals-agent" "version" "{_AnyDeals_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Anydeals Agent")
        ...
        msg_id = f"<AnyDeals-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
