...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_AnyDeals_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /AnyDeals prefix). A single regex
            ...
            # manifest (see `AnyDeals slack manifest`). In Socket Mode, Slack
            ...
            from AnyDeals_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/AnyDeals$")

            @self._app.command(_slash_pattern)
            async def handle_AnyDeals_command(ack, command):
            ...
            for _action_id in (
                "AnyDeals_approve_once",
                "AnyDeals_approve_session",
                "AnyDeals_approve_always",
                "AnyDeals_deny",
            ):
            ...
            for _action_id in (
                "AnyDeals_confirm_once",
                "AnyDeals_confirm_always",
                "AnyDeals_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: Anydeals handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Anydeals session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "AnyDeals_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "AnyDeals_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "AnyDeals_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "AnyDeals_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "AnyDeals_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "AnyDeals_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "AnyDeals_confirm_once": "once",
            "AnyDeals_confirm_always": "always",
            "AnyDeals_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "AnyDeals_approve_once": "once",
            "AnyDeals_approve_session": "session",
            "AnyDeals_approve_always": "always",
            "AnyDeals_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/AnyDeals <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/AnyDeals
        ...
        if slash_name in {"AnyDeals", ""}:
            # Legacy /AnyDeals <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from AnyDeals_cli.commands import slack_subcommand_map
        ...
        # "Running /cmd…" ack shown by handle_AnyDeals_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/AnyDeals <question>" must produce public replies so
