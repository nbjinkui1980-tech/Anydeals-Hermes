    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if AnyDeals_home_str:
            takeover_path = Path(AnyDeals_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(AnyDeals_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded AnyDeals-agent but never re-ran ``AnyDeals setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../AnyDeals-gateway.service"
    ...
    # that's the common case for AnyDeals.
    ...
    # constant in AnyDeals_cli/gateway.py.
    headroom = 30.0
