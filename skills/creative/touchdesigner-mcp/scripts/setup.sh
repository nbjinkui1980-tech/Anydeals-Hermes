...
ANYDEALS_HOME_DIR="${ANYDEALS_HOME:-$HOME/.AnyDeals}"
ANYDEALS_CFG="${ANYDEALS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Anydeals config has twozero_td MCP entry ──
if [[ ! -f "$ANYDEALS_CFG" ]]; then
    echo -e " ${FAIL} Anydeals config not found at ${ANYDEALS_CFG}"
    manual_steps+=("Create ${ANYDEALS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ANYDEALS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Anydeals config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Anydeals config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$ANYDEALS_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ANYDEALS_CFG} manually"); }
    manual_steps+=("Restart Anydeals session to pick up config change")
fi
