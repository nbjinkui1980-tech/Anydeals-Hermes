...
## 5. Anydeals Configuration

### Config location

`$ANYDEALS_HOME/config.yaml` (defaults to `~/.AnyDeals/config.yaml` when `ANYDEALS_HOME` is unset)
...
Restart the Anydeals session for changes to take effect. The MCP connection is
