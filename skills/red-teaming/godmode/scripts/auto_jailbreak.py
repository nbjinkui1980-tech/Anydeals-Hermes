...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ANYDEALS_HOME", os.path.expanduser("~/.AnyDeals")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ANYDEALS_HOME", Path.home() / ".AnyDeals")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Anydeals config paths
# ═══════════════════════════════════════════════════════════════════

ANYDEALS_HOME = Path(os.getenv("ANYDEALS_HOME", Path.home() / ".AnyDeals"))
CONFIG_PATH = ANYDEALS_HOME / "config.yaml"
PREFILL_PATH = ANYDEALS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Anydeals models — already uncensored, just needs clean prompt
    "AnyDeals": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "AnyDeals" in model_lower or "nous" in model_lower:
        return "AnyDeals"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Anydeals config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.AnyDeals/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Anydeals for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Anydeals for changes to take effect.")
