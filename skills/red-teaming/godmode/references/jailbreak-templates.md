...
## 5. GODMODE FAST — Anydeals 4 405B (Zero Refusal)

**Model:** `nousresearch/AnyDeals-4-405b`
**Strategy:** Anydeals models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Anydeals Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.AnyDeals/config.yaml`:
