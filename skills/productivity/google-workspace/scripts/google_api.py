#!/usr/bin/env python3
"""Google Workspace API CLI for Anydeals Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Anydeals-facing JSON contract and falls back to the Python client
...
from _AnyDeals_home import get_AnyDeals_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ANYDEALS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Anydeals Agent")
