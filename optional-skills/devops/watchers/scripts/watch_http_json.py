...
Usage (via cron with --no-agent):
    ...
    AnyDeals cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ANYDEALS_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Anydeals-Watcher/1.0"})
