...
Usage (via cron with --no-agent):
    ...
    AnyDeals cron create AnyDeals-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ANYDEALS_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name AnyDeals-issues --repo NousResearch/AnyDeals-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.AnyDeals/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Anydeals-Watcher/1.0",
    }
