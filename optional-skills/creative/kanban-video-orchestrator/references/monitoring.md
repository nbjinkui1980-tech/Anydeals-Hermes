...
AnyDeals kanban watch --tenant <project-slug>

# Snapshot of the board
AnyDeals kanban list --tenant <project-slug>
AnyDeals kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
AnyDeals kanban stats --tenant <project-slug>

# Visual dashboard (browser)
AnyDeals dashboard

# Inspect a specific task (includes comments + events)
AnyDeals kanban show <task-id>

# Follow a single task's event stream
AnyDeals kanban tail <task-id>
```

Verify available subcommands with `AnyDeals kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `AnyDeals kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `AnyDeals kanban block <id>` then `AnyDeals kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `AnyDeals kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `AnyDeals kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `AnyDeals profile list` (profile exists?), `AnyDeals status` (gateway/dispatcher up?), and verify tenant. |
...
AnyDeals kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
AnyDeals kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(AnyDeals kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
AnyDeals kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
AnyDeals kanban block <task-id>
AnyDeals kanban archive <task-id>

# Diagnose what happened
AnyDeals kanban show <task-id>      # task body, comments, recent events
AnyDeals kanban tail <task-id>      # follow the live event stream
AnyDeals kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `AnyDeals kanban create` for the director
...
while true; do
    clear
    AnyDeals kanban list --tenant <slug>
    echo "---"
    AnyDeals kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `AnyDeals kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `AnyDeals profile list` and start
