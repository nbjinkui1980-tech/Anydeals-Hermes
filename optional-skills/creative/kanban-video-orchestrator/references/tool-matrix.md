...
Maps each role archetype to the Anydeals skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public AnyDeals-agent
...
## Anydeals skills relevant to video production

### Visual / rendering skills (`AnyDeals-agent/skills/creative/`)
...
### Design / pre-production skills (`AnyDeals-agent/skills/creative/`)
...
### Audio / media skills (`AnyDeals-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`AnyDeals-agent/skills/devops/`)
...
These are **not** Anydeals skills but external CLIs / APIs that profiles invoke.
...
## Built-in Anydeals tools for media review

These are native Anydeals tools — not invoked via terminal but through their own
...
key is present in `~/.AnyDeals/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Anydeals profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.AnyDeals/.env` → environment variable.
...
When a new Anydeals-public video skill ships:
