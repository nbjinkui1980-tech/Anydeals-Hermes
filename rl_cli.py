...
from AnyDeals_constants import OPENROUTER_BASE_URL, get_AnyDeals_home
...
if tinker_atropos_dir.exists():
    ...
    os.environ['ANYDEALS_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to AnyDeals-agent directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.AnyDeals/config.yaml if not provided)
    ...
    """
    # Load config from ~/.AnyDeals/config.yaml
    config = load_AnyDeals_config()
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.AnyDeals/.env")
            else:
