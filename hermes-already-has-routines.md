# Anydeals Agent Has Had "Routines" Since March
...
Anydeals equivalent — works today:
```bash
...
Anydeals equivalent — works today:
```bash
AnyDeals webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Anydeals implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Anydeals Agent |
|---|---|---|
...
## Things Anydeals Does That Routines Can't
...
Anydeals has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Anydeals Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install AnyDeals-agent
AnyDeals setup
```
...
```bash
AnyDeals gateway setup    # enable webhooks
AnyDeals webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [AnyDeals-agent.nousresearch.com/docs/guides/automation-templates](https://AnyDeals-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [AnyDeals-agent.nousresearch.com](https://AnyDeals-agent.nousresearch.com)

GitHub: [github.com/NousResearch/AnyDeals-agent](https://github.com/NousResearch/AnyDeals-agent)

---

*Anydeals Agent is built by [JINKUI](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
