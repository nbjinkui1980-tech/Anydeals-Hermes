...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/AnyDeals-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    AnyDeals_meta = (fm.get("metadata") or {}).get("AnyDeals") or {}
    tags = AnyDeals_meta.get("tags") or []
    related = AnyDeals_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `AnyDeals skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Anydeals Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Anydeals ships with a large built-in skill library copied into `~/.AnyDeals/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Anydeals also syncs bundled skills on `AnyDeals update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.AnyDeals/skills/` tree, it is still shipped with Anydeals; restore it with `AnyDeals skills reset <name> --restore`.",
        "",
