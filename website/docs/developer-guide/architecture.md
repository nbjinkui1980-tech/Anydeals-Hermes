...
sidebar_position: 1
title: "Architecture"
description: "Anydeals Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Anydeals Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ AnyDeals_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AnydealsCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── AnyDeals_state.py           # SQLite session/state database with FTS5
├── AnyDeals_constants.py       # ANYDEALS_HOME, profile-aware paths
...
│
├── AnyDeals_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `AnyDeals` subcommands (large file)
...
│   ├── skills_config.py      # AnyDeals skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # AnyDeals tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # AnyDeals gateway start/stop
│
...
```text
User input → AnydealsCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .AnyDeals.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.AnyDeals/plugins/` (user), `.AnyDeals/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `AnyDeals plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-AnyDeals-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Anydeals as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`AnyDeals -p <name>`) gets its own ANYDEALS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
