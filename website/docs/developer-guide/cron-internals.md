...
description: "How Anydeals stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `AnyDeals_cli/cron.py` | CLI `AnyDeals cron` subcommands |
...
Jobs are stored in `~/.AnyDeals/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `AnyDeals cron` commands are run or during active CLI sessions.
...
```python
# ~/.AnyDeals/scripts/check_competitors.py
...
2. **Environment variable** — `ANYDEALS_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.AnyDeals/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `AnyDeals cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `AnyDeals cron` CLI provides direct job management:

```bash
AnyDeals cron list                    # Show all jobs
AnyDeals cron create                  # Interactive job creation (alias: add)
AnyDeals cron edit <job_id>           # Edit job configuration
AnyDeals cron pause <job_id>          # Pause a running job
AnyDeals cron resume <job_id>         # Resume a paused job
AnyDeals cron run <job_id>            # Trigger immediate execution
AnyDeals cron remove <job_id>         # Delete a job
```
