...
[PR #12550](https://github.com/NousResearch/AnyDeals-agent/pull/12550) proposed a
...
(`AnyDeals-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Anydeals `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `AnyDeals-acp`, `AnyDeals-api-server`, core toolsets (gated on CDP reachability)
...
- `AnyDeals_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
