...
description: "Run Anydeals Agent directly on an Android phone with Termux"
---

# Anydeals on Android with Termux

This is the tested path for running Anydeals Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Anydeals from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Anydeals now ships a Termux-aware installer path:
...
- links `AnyDeals` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `AnyDeals` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/AnyDeals" "$PREFIX/bin/AnyDeals"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `AnyDeals` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.AnyDeals/.env`.
...
### `AnyDeals doctor` says ripgrep or Node is missing
