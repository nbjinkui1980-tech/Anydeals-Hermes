...
description: "Diagnose and fix common Anydeals cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`AnyDeals gateway` for foreground, or `AnyDeals gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `AnyDeals cron tick`.
...
```bash
date
AnyDeals cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.AnyDeals/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.AnyDeals/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.AnyDeals/.env` |
...
| `local` | Write access to `~/.AnyDeals/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `AnyDeals cron list` for updated `last_error` field (if available).
...
```bash
AnyDeals skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `AnyDeals skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `AnyDeals skills list`.
...
2. `~/.AnyDeals/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `AnyDeals cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Anydeals config directory). Verify:
```bash
ls ~/.AnyDeals/scripts/your-script.py   # Must exist
AnyDeals cron edit <job_id> --script ~/.AnyDeals/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `AnyDeals skills install <skill-name>`.
...
```bash
ps aux | grep AnyDeals
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.AnyDeals/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.AnyDeals/cron/jobs.json
chmod 600 ~/.AnyDeals/cron/jobs.json   # Your user should own it
```
...
```bash
AnyDeals cron list                    # Show all jobs, states, next_run times
AnyDeals cron run <job_id>            # Schedule for next tick (for testing)
AnyDeals cron edit <job_id>           # Fix configuration issues
AnyDeals logs                         # View recent Anydeals logs
AnyDeals skills list                  # Verify installed skills
```
...
1. Run the job with `AnyDeals cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.AnyDeals/logs/agent.log` for scheduler messages and `~/.AnyDeals/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/AnyDeals-agent](https://github.com/NousResearch/AnyDeals-agent) with:
