...
This tutorial walks you through setting up a Telegram bot powered by Anydeals Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Anydeals Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.AnyDeals/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Anydeals itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Anydeals Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_AnyDeals_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Anydeals Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.AnyDeals/.env`:
...
```
[Gateway] Starting Anydeals Gateway...
...
# View live logs
journalctl --user -u AnyDeals-gateway -f
...
AnyDeals gateway start
AnyDeals gateway stop
tail -f ~/.AnyDeals/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `AnyDeals gateway install` to update the plist.
:::
...
```bash
AnyDeals gateway stop && AnyDeals gateway start
```
...
AnyDeals pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.AnyDeals/.env`:
...
Control how much detail the bot shows when using tools. In `~/.AnyDeals/config.yaml`:
...
Customize how the bot communicates by editing `~/.AnyDeals/SOUL.md`:

For a full guide, see [Use SOUL.md with Anydeals](/docs/guides/use-soul-with-AnyDeals).
...
```bash
# From the CLI
AnyDeals cron list          # View all scheduled jobs
AnyDeals cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u AnyDeals-gateway` (Linux) or `~/.AnyDeals/logs/gateway.log` (macOS) |
| Cron job output | `~/.AnyDeals/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.AnyDeals/cron/jobs.json` |
| Pairing data | `~/.AnyDeals/pairing/` |
| Session history | `~/.AnyDeals/sessions/` |
