---
sidebar_position: 6
title: "Use MCP with Anydeals"
description: "A practical guide to connecting MCP servers to Anydeals Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Anydeals

This guide shows how to actually use MCP with Anydeals Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Anydeals tool
- you want Anydeals to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Anydeals to internal APIs, databases, or company systems without modifying Anydeals core

Do not use MCP when:
- a built-in Anydeals tool already solves the job well
...
- Anydeals remains the agent
- MCP servers contribute tools
- Anydeals discovers those tools at startup or reload time
...
If you installed Anydeals with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Anydeals banner/status should show MCP integration when configured
- ask Anydeals what tools it has available
...
## WSL2: bridge Anydeals in WSL to Windows Chrome

This is the practical setup when:

- Anydeals runs inside WSL2
...
In this setup, Anydeals does **not** connect to Chrome directly. Instead:

- Anydeals runs in WSL
- Anydeals starts a local stdio MCP server
...
```text
Anydeals (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Anydeals stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Anydeals core browser transport
...
```bash
AnyDeals mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
AnyDeals mcp test chrome-devtools-win
```

Then start a fresh Anydeals session or run:
...
Once loaded, Anydeals can use the MCP-prefixed browser tools directly. For example:
...
If Anydeals runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Anydeals from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Anydeals from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Anydeals:
...
2. Anydeals-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Anydeals will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Anydeals to reason over a bounded workspace.
...
Now Anydeals can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Anydeals core.
...
Because Anydeals now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
