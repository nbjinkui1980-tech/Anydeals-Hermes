...
```bash
AnyDeals teams-pipeline validate
```
...
```bash
AnyDeals teams-pipeline token-health
AnyDeals teams-pipeline token-health --force-refresh
```
...
```bash
AnyDeals teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
AnyDeals teams-pipeline maintain-subscriptions
AnyDeals teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Anydeals cron (recommended if you already run the Anydeals gateway)

Anydeals ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.AnyDeals/scripts/`. First create the script:

```bash
mkdir -p ~/.AnyDeals/scripts
cat > ~/.AnyDeals/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec AnyDeals teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.AnyDeals/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
AnyDeals cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/AnyDeals-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Anydeals Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=AnyDeals
EnvironmentFile=/etc/AnyDeals/env
ExecStart=/usr/local/bin/AnyDeals teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/AnyDeals-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Anydeals Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now AnyDeals-teams-pipeline-maintain.timer
systemctl list-timers AnyDeals-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/AnyDeals teams-pipeline maintain-subscriptions >> /var/log/AnyDeals/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.AnyDeals/.env` at the top of a wrapper script that crontab calls.
...
```bash
AnyDeals teams-pipeline subscriptions   # should show expirationDateTime advanced
AnyDeals teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
AnyDeals teams-pipeline list
AnyDeals teams-pipeline list --status failed
AnyDeals teams-pipeline show <job-id>
```

### Replay a stored job

```bash
AnyDeals teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
AnyDeals teams-pipeline fetch --meeting-id <meeting-id>
AnyDeals teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `AnyDeals teams-pipeline maintain-subscriptions --dry-run`
- inspect `AnyDeals teams-pipeline list --status failed`
...
- run `AnyDeals teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `AnyDeals teams-pipeline run`
...
- [ ] `AnyDeals teams-pipeline validate` returns an OK snapshot
- [ ] `AnyDeals teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Anydeals cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
