...
Copy-paste recipes for common automation patterns. Each template uses Anydeals's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`AnyDeals webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `AnyDeals webhook subscribe`) |
...
```bash
AnyDeals cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/AnyDeals-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/AnyDeals-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
AnyDeals webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
AnyDeals cron create "0 9 * * 1" \
  "Scan the NousResearch/AnyDeals-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/AnyDeals-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (AnyDeals_cli/commands.py, AnyDeals_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (AnyDeals_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
AnyDeals cron create "0 6 * * *" \
  "Run a dependency security audit on the AnyDeals-agent project.

1. cd ~/.AnyDeals/AnyDeals-agent && source .venv/bin/activate
...
```bash
AnyDeals webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
AnyDeals webhook subscribe alert-triage \
...
```python title="~/.AnyDeals/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Anydeals-Monitor/1.0"})
...
```bash
AnyDeals cron create "every 30m" \
  ...
  --script ~/.AnyDeals/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
AnyDeals cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Anydeals Agent development." \
...
```bash
AnyDeals webhook subscribe github-issues \
  --events "issues" \
...
```bash
AnyDeals webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
AnyDeals webhook subscribe stripe-payments \
...
```bash
AnyDeals cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the AnyDeals-agent codebase.
