...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Anydeals Agent — no API key required"
---

# MiniMax OAuth

Anydeals Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Anydeals automatically refreshes your session.
...
# → Anydeals opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.AnyDeals/auth.json` and are refreshed automatically before each session.
...
```bash
AnyDeals auth add minimax-cn --type oauth   # if OAuth is supported on your CN account
# or simpler:
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.AnyDeals/.env
```
...
```bash
AnyDeals auth add minimax-oauth --no-browser
```

Anydeals will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Anydeals implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. Anydeals generates a PKCE verifier / challenge pair and a random state value.
...
4. Anydeals polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.AnyDeals/auth.json` under the `minimax-oauth` key.
...
```bash
AnyDeals config set model MiniMax-M2.7
AnyDeals config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.AnyDeals/config.yaml` will contain entries similar to:
...
```bash
AnyDeals --provider minimax-oauth    # canonical
AnyDeals --provider minimax-portal   # alias
AnyDeals --provider minimax-global   # alias
AnyDeals --provider minimax_oauth    # alias (underscore form)
```
...
```bash
ANYDEALS_INFERENCE_PROVIDER=minimax-oauth AnyDeals
```
...
Anydeals refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Anydeals marks the session as requiring re-login.

**Fix:** run `AnyDeals auth add minimax-oauth` again to start a fresh login.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Anydeals raises a timeout error.

**Fix:** re-run `AnyDeals auth add minimax-oauth` (or `AnyDeals model`). The flow starts fresh.

### State mismatch (possible CSRF)

Anydeals detected that the `state` value returned by the authorization server does not match what it sent.
...
If `AnyDeals` cannot open a browser window, use `--no-browser`:

```bash
AnyDeals auth add minimax-oauth --no-browser
```

Anydeals prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `AnyDeals model` and select MiniMax (OAuth), or run `AnyDeals auth add minimax-oauth`.
...
```bash
AnyDeals auth remove minimax-oauth
```
...
- [AnyDeals doctor](../reference/cli-commands.md)
