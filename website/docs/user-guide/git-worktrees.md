...
title: "Git Worktrees"
description: "Run multiple Anydeals agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Anydeals Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Anydeals so each session has a clean, isolated working directory.

## Why Use Worktrees with Anydeals?

Anydeals treats the **current working directory** as the project root:

- CLI: the directory where you run `AnyDeals` or `AnyDeals chat`
...
git worktree add ../repo-feature feature/AnyDeals-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/AnyDeals-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Anydeals there:

```bash
cd ../repo-feature

# Start Anydeals in the worktree
AnyDeals
```
...
git worktree add ../repo-experiment-a feature/AnyDeals-a
git worktree add ../repo-experiment-b feature/AnyDeals-b
```
...
Each Anydeals process:

- Works on its own branch (`feature/AnyDeals-a` vs `feature/AnyDeals-b`).
...
- Anydeals checkpoint data under `~/.AnyDeals/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Anydeals experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/AnyDeals-checkpoints-docs`, `feature/AnyDeals-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Anydeals from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `AnyDeals -w` (Automatic Worktree Mode)

Anydeals has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `AnyDeals/AnyDeals-<hash>`).
...
```bash
AnyDeals -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `AnyDeals -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Anydeals session its own clean checkout.
