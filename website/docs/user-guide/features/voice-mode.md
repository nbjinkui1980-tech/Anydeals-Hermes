...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Anydeals Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Anydeals Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Anydeals](/docs/guides/use-voice-mode-with-AnyDeals).

## Prerequisites

Before using voice features, make sure you have:

1. **Anydeals Agent installed** — `pip install AnyDeals-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `AnyDeals model` or set your preferred provider credentials in `~/.AnyDeals/.env`
3. **A working base setup** — run `AnyDeals` to verify the agent responds to text before enabling voice

:::tip
The `~/.AnyDeals/` directory and default `config.yaml` are created automatically the first time you run `AnyDeals`. You only need to create `~/.AnyDeals/.env` manually for API keys.
:::
...
pip install "AnyDeals-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "AnyDeals-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "AnyDeals-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "AnyDeals-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`AnyDeals chat`) and the **TUI** (`AnyDeals --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.AnyDeals/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
AnyDeals                # Start the interactive CLI
```
...
1. Start the CLI with `AnyDeals` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.AnyDeals/config.yaml` (default: `ctrl+b`).
:::
...
```bash
AnyDeals gateway        # Start the gateway (connects to configured platforms)
AnyDeals gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@AnyDealsbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.AnyDeals/.env`:
```bash
...
```bash
AnyDeals gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.AnyDeals/.env`
...
- Review gateway logs: `tail -f ~/.AnyDeals/logs/gateway.log`
