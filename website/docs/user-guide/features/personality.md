...
description: "Customize Anydeals Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Anydeals Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ANYDEALS_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Anydeals is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Anydeals now seeds a default `SOUL.md` automatically in:

```text
~/.AnyDeals/SOUL.md
```

More precisely, it uses the current instance's `ANYDEALS_HOME`, so if you run Anydeals with a custom home directory, it will use:

```text
$ANYDEALS_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Anydeals creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Anydeals loads `SOUL.md` only from `ANYDEALS_HOME`
- Anydeals does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Anydeals falls back to a built-in default identity
...
If Anydeals loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ANYDEALS_HOME`, the personality belongs to the Anydeals instance itself.

That also makes it easier to teach users:
- "Edit `~/.AnyDeals/SOUL.md` to change Anydeals' default personality."
...
- how Anydeals should handle uncertainty, disagreement, or ambiguity
...
## What Anydeals injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Anydeals falls back to a built-in default identity ("You are Anydeals Agent, an intelligent AI assistant created by JINKUI..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Anydeals ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Anydeals, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Anydeals its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.AnyDeals/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.AnyDeals/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-AnyDeals)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Anydeals speaks
- `display.skin` and `/skin` affect how Anydeals looks in the terminal
