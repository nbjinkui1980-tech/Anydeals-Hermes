...
Anydeals Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
```
AnyDeals computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `AnyDeals computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `AnyDeals tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
3. Grant macOS permissions when prompted:
   - **System Settings → Privacy & Security → Accessibility** → allow the
     terminal (or Anydeals app).
   - **System Settings → Privacy & Security → Screen Recording** → allow
     the same.
4. Start a session with the toolset enabled:
   ```
   AnyDeals -t computer_use chat
   ```
   or add `computer_use` to your enabled toolsets in `~/.AnyDeals/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Anydeals refreshes the binary in two
places so you don't get stuck on a stale release:

- **`AnyDeals update`** — when you update Anydeals itself, if `cua-driver` is
...
- **`AnyDeals computer-use install --upgrade`** — manual force-refresh.
...
`AnyDeals computer-use status` shows the installed version next to the
binary path.
...
Anydeals applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.AnyDeals/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Anydeals applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `ANYDEALS_CUA_DRIVER_VERSION`
...
```
ANYDEALS_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
ANYDEALS_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
ANYDEALS_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`AnyDeals computer-use install` to fetch the cua-driver binary, or run
`AnyDeals tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/AnyDeals-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
