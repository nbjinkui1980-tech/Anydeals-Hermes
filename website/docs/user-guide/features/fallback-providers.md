...
Anydeals Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Anydeals can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`AnyDeals fallback` reuses the provider picker from `AnyDeals model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.

If you'd rather edit the YAML directly, add a `fallback_model` section to `~/.AnyDeals/config.yaml`:
...
`fallback_model` (singular) is the legacy single-fallback key — Anydeals still honors it for back-compat. `fallback_providers` (plural, list) supports multiple fallbacks tried in order; `AnyDeals fallback` writes to this key. When both are set, Anydeals merges them with `fallback_providers` taking priority.
:::
...
| Nous Portal | `nous` | `AnyDeals auth` (OAuth) |
| OpenAI Codex | `openai-codex` | `AnyDeals model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `AnyDeals model` (Google OAuth; optional: `ANYDEALS_GEMINI_PROJECT_ID`) |
...
| Qwen Portal (OAuth) | `qwen-oauth` | `AnyDeals model` (Qwen Portal OAuth; optional: `ANYDEALS_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `AnyDeals model` (MiniMax portal OAuth) |
...
Anydeals uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `AnyDeals kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), Anydeals tries providers in order until one works:
...
If the resolved provider fails at call time, Anydeals also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
For `auxiliary.session_search`, Anydeals also supports:
...
| `"codex"` | Force Codex OAuth | `AnyDeals model` → Codex |
...
`base_url` takes precedence over `provider`. Anydeals uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, Anydeals drops middle conversation turns without generating a summary rather than failing the session.
