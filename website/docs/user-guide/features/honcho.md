...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Anydeals's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Anydeals instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
AnyDeals memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.AnyDeals/.env
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$ANYDEALS_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `AnyDeals` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Anydeals syncs them back at session init.
...
The `AnyDeals honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `AnyDeals memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
AnyDeals honcho status          # Connection status, config, and key settings
AnyDeals honcho setup           # Redirects to `AnyDeals memory setup`
AnyDeals honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
AnyDeals honcho peer            # Show or update peer names + dialectic reasoning level
AnyDeals honcho mode            # Show or set recall mode (hybrid/context/tools)
AnyDeals honcho tokens          # Show or set token budget for context and dialectic
AnyDeals honcho identity        # Seed or show the AI peer's Honcho identity
AnyDeals honcho sync            # Sync Honcho config to all existing profiles
AnyDeals honcho peers           # Show peer identities across all profiles
AnyDeals honcho sessions        # List known Honcho session mappings
AnyDeals honcho map             # Map current directory to a Honcho session name
AnyDeals honcho enable          # Enable Honcho for the active profile
AnyDeals honcho disable         # Disable Honcho for the active profile
AnyDeals honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `AnyDeals honcho`

If you previously used the standalone `AnyDeals honcho setup`:
...
No re-login or re-setup needed. Run `AnyDeals memory setup` and select "honcho" — the wizard detects your existing config.
