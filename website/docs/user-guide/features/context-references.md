...
Type `@` followed by a reference to inject content directly into your message. Anydeals expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Anydeals env: `$ANYDEALS_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ANYDEALS_HOME/skills/.hub/`
