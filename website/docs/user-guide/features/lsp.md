...
Anydeals runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Anydeals
...
1. Anydeals captures a baseline of current diagnostics for the file.
...
…). Anydeals auto-detects the binary on PATH or in
`<ANYDEALS_HOME>/lsp/bin/`.
...
`node_modules` tree — Anydeals installs both packages together when you
run `AnyDeals lsp install typescript` or auto-install fires on first
use.

## CLI

```
AnyDeals lsp status          # service state + per-server install status
AnyDeals lsp list            # registry, optionally --installed-only
AnyDeals lsp install <id>    # eagerly install one server
AnyDeals lsp install-all     # try every server with a known recipe
AnyDeals lsp restart         # tear down running clients
AnyDeals lsp which <id>      # print resolved binary path
```

`AnyDeals lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ANYDEALS_HOME>/lsp/bin
...
When `install_strategy: auto`, Anydeals installs binaries into
`<ANYDEALS_HOME>/lsp/bin/`. NPM packages land in
`<ANYDEALS_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Anydeals-owned and is
...
Servers are kept alive for the life of the Anydeals process. There's
...
**`AnyDeals lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ANYDEALS_HOME>/lsp/bin/`. Run
`AnyDeals lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `AnyDeals lsp status`**
...
When `AnyDeals lsp status` shows a `Backend warnings` section, install
...
`~/.AnyDeals/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.AnyDeals/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `AnyDeals lsp restart` to clear the set;
