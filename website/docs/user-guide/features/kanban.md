...
description: "Durable SQLite-backed task board for coordinating multiple Anydeals profiles"
---
...
Anydeals Kanban is a durable task board, shared across all your Anydeals profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.AnyDeals/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.AnyDeals/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `AnyDeals kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `AnyDeals kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/AnyDeals-kanban-v1-spec.pdf` in the repository.
...
- **Workspace** — the directory a worker operates in. Three kinds:
  - `scratch` (default) — fresh tmp dir under `~/.AnyDeals/kanban/workspaces/<id>/` (or `~/.AnyDeals/kanban/boards/<slug>/workspaces/<id>/` on non-default boards).
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `ANYDEALS_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
AnyDeals kanban boards list

# Create a new board.
AnyDeals kanban boards create atm10-server \
    --name "ATM10 Server" \
...
AnyDeals kanban --board atm10-server list
AnyDeals kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
AnyDeals kanban boards switch atm10-server
AnyDeals kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
AnyDeals kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `ANYDEALS_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.AnyDeals/kanban/current` — the slug persisted by `AnyDeals kanban
   boards switch`.
4. `default`.
...
`AnyDeals dashboard` → Kanban tab shows a board switcher at the top as soon
...
AnyDeals kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
AnyDeals kanban watch

# 5. See the board (you)
AnyDeals kanban list
AnyDeals kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `AnyDeals kanban show t_abcd`.
...
Override the config flag at runtime via `ANYDEALS_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `AnyDeals gateway
...
until one comes up — `AnyDeals kanban create` warns about this at creation
time.

Running `AnyDeals kanban daemon` as a separate process is **deprecated**;
...
AnyDeals kanban create "nightly ops review" \
    --assignee ops \
...
```bash
AnyDeals kanban complete t_abc t_def t_hij --result "batch wrap"
AnyDeals kanban archive  t_abc t_def t_hij
AnyDeals kanban unblock  t_abc t_def
AnyDeals kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `AnyDeals kanban`.** When the dispatcher spawns a worker it sets `ANYDEALS_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `AnyDeals kanban` CLI.
...
kanban_show()                                     # no args — uses ANYDEALS_KANBAN_TASK
...
### Why tools instead of shelling to `AnyDeals kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `AnyDeals kanban complete` *inside* the container, where `AnyDeals` isn't installed and `~/.AnyDeals/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.AnyDeals/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `AnyDeals chat` session has zero `kanban_*` tools in its schema. The `check_fn` on each tool only returns True when `ANYDEALS_KANBAN_TASK` is set, which only happens when the dispatcher spawned this process. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/AnyDeals_cli/test_kanban_db.py -q"],
...
2. `cd $ANYDEALS_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
AnyDeals -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
AnyDeals -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
AnyDeals kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

AnyDeals kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
AnyDeals -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
AnyDeals -p orchestrator skills reset kanban-orchestrator --restore
```
...
- A **Kanban** tab showing one column per status: `triage`, `todo`, `ready`, `running`, `blocked`, `done` (plus `archived` when the toggle is on).
  - `triage` is the parking column for rough ideas a specifier is expected to flesh out. Tasks created with `AnyDeals kanban create --triage` (or via the Triage column's inline create) land here and the dispatcher leaves them alone until a human or specifier promotes them to `todo` / `ready`. Run `AnyDeals kanban specify <id>` to have the auxiliary LLM expand a triage task into a concrete spec (title + body with goal, approach, acceptance criteria) and promote it to `todo` in one shot; `--all` sweeps every triage task at once. Configure which model runs the specifier under `auxiliary.triage_specifier` in `config.yaml`.
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.AnyDeals/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
Tasks in `~/.AnyDeals/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `AnyDeals -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard Anydeals dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
AnyDeals kanban init                                     # create kanban.db + print daemon hint
AnyDeals kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
AnyDeals kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived] [--json]
AnyDeals kanban show <id> [--json]
AnyDeals kanban assign <id> <profile>                    # or 'none' to unassign
AnyDeals kanban link <parent_id> <child_id>
AnyDeals kanban unlink <parent_id> <child_id>
AnyDeals kanban claim <id> [--ttl SECONDS]
AnyDeals kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
AnyDeals kanban complete <id>... [--result "..."]
AnyDeals kanban block <id> "<reason>" [--ids <id>...]
AnyDeals kanban unblock <id>...
AnyDeals kanban archive <id>...

AnyDeals kanban tail <id>                                # follow a single task's event stream
AnyDeals kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
AnyDeals kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
AnyDeals kanban runs <id> [--json]                       # attempt history (one row per run)
AnyDeals kanban assignees [--json]                       # profiles on disk + per-assignee task counts
AnyDeals kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
AnyDeals kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `AnyDeals gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
AnyDeals kanban stats [--json]                           # per-status + per-assignee counts
AnyDeals kanban log <id> [--tail BYTES]                  # worker log from ~/.AnyDeals/kanban/logs/
AnyDeals kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
AnyDeals kanban notify-list [<id>] [--json]
AnyDeals kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
AnyDeals kanban context <id>                             # what a worker sees
AnyDeals kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
AnyDeals kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
Every `AnyDeals kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `AnyDeals chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `AnyDeals_cli.kanban.run_slash()` entry point that reuses the `AnyDeals kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `AnyDeals kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.AnyDeals/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`AnyDeals kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `AnyDeals kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/AnyDeals-kanban-v1-spec.pdf`.
...
```bash
AnyDeals kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$ANYDEALS_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
AnyDeals kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
AnyDeals kanban notify-list
AnyDeals kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
AnyDeals kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
AnyDeals kanban runs t_abcd
...
**Bulk close caveat.** `AnyDeals kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`AnyDeals kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `AnyDeals kanban heartbeat $TASK` to signal liveness during long operations. |
...
`AnyDeals kanban tail <id>` shows these for a single task. `AnyDeals kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.AnyDeals/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/AnyDeals-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
