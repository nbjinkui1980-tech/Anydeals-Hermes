...
| | |
|---|---|
| Source | Optional — install with `AnyDeals skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Anydeals or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Anydeals. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.AnyDeals/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.AnyDeals/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Anydeals integration, either:

- configure the server in `~/.AnyDeals/config.yaml` using the `native-mcp` skill, or
...
### Anydeals cannot see the deployed server

The server-building part may be correct while the Anydeals config is not. Load the `native-mcp` skill and configure the server in `~/.AnyDeals/config.yaml`, then restart Anydeals.
