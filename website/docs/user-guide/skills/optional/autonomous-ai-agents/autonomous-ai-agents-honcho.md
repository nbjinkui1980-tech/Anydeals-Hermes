...
description: "Configure and use Honcho memory with Anydeals -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Anydeals -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `AnyDeals skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Anydeals

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Anydeals profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/AnyDeals#running-honcho-locally-with-AnyDeals

### Verify

```bash
AnyDeals honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Anydeals profile's AI peer
...
Honcho models conversations as interactions between **peers**. Anydeals creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Anydeals instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Anydeals run |
| `global` | Single session across all directories |

Manual override: `AnyDeals honcho map my-project-name`
...
Each Anydeals profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block AnyDeals.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `AnyDeals.coder` host block in `honcho.json`
...
```bash
AnyDeals honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Anydeals when Honcho memory is active.
...
Config file: `$ANYDEALS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `AnyDeals honcho setup`. Ensure `memory.provider: honcho` is in `~/.AnyDeals/config.yaml`.

### Memory not persisting across sessions
Check `AnyDeals honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `AnyDeals profile create <name> --clone`. For existing profiles: `AnyDeals honcho sync`.
...
| Command | Description |
|---------|-------------|
| `AnyDeals honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `AnyDeals honcho status` | Show resolved config, connection test, peer info for active profile |
| `AnyDeals honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `AnyDeals honcho disable` | Disable Honcho for the active profile |
| `AnyDeals honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `AnyDeals honcho peers` | Show peer identities across all profiles |
| `AnyDeals honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `AnyDeals honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `AnyDeals honcho sessions` | List known directory-to-session-name mappings |
| `AnyDeals honcho map <name>` | Map current working directory to a Honcho session name |
| `AnyDeals honcho identity` | Seed AI peer identity or show both peer representations |
| `AnyDeals honcho sync` | Create host blocks for all Anydeals profiles that don't have one yet |
| `AnyDeals honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Anydeals + Honcho |
| `AnyDeals memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `AnyDeals memory status` | Show active memory provider and config |
| `AnyDeals memory off` | Disable external memory provider |
