...
| | |
|---|---|
| Source | Optional — install with `AnyDeals skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Anydeals Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Anydeals terminal calls (desktop app flow only)
...
### Service Account (recommended for Anydeals)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.AnyDeals/.env` (the skill will prompt for this on first load).
...
## Anydeals Execution Pattern (desktop app flow)

Anydeals terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/AnyDeals-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/AnyDeals-op.sock"
