...
| | |
|---|---|
| Source | Optional — install with `AnyDeals skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Anydeals Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.AnyDeals/.env`:
...
```bash
CANVAS="python $ANYDEALS_HOME/skills/productivity/canvas/scripts/canvas_api.py"
