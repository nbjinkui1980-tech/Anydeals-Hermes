...
| | |
|---|---|
| Source | Optional — install with `AnyDeals skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ANYDEALS_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ANYDEALS_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ANYDEALS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client AnyDeals
```
...
```bash
PUBLISH="${ANYDEALS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client AnyDeals
```
...
```bash
DRIVE="${ANYDEALS_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `AnyDeals`)    |
