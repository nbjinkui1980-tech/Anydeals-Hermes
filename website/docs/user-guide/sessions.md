...
Anydeals Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.AnyDeals/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.AnyDeals/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`AnyDeals` or `AnyDeals chat`) |
...
AnyDeals -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
AnyDeals -c "my project"   # → resumes "my project #3"
```
...
AnyDeals --resume 20250305_091523_a1b2c3d4
AnyDeals -r 20250305_091523_a1b2c3d4

# Resume by title
AnyDeals --resume "refactoring auth"

# Or with the chat subcommand
AnyDeals chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `AnyDeals sessions list`.

### Conversation Recap on Resume

When you resume a session, Anydeals displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Anydeals session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.AnyDeals/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `AnyDeals -r "<title>"` from the shell) and pick up where the platform left off.
...
Anydeals automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `AnyDeals sessions list` or `AnyDeals sessions browse`.
...
```bash
AnyDeals sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Anydeals creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`AnyDeals -c "my project"`), it automatically picks the most recent session in the lineage.
...
Anydeals provides a full set of session management commands via `AnyDeals sessions`:
...
# Filter by platform
AnyDeals sessions list --source telegram

# Show more sessions
AnyDeals sessions list --limit 50
```
...
AnyDeals sessions export backup.jsonl

# Export sessions from a specific platform
AnyDeals sessions export telegram-history.jsonl --source telegram

# Export a single session
AnyDeals sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
AnyDeals sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
AnyDeals sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
AnyDeals sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
AnyDeals sessions prune

# Custom age threshold
AnyDeals sessions prune --older-than 30

# Only prune sessions from a specific platform
AnyDeals sessions prune --source telegram --older-than 60

# Skip confirmation
AnyDeals sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`AnyDeals insights`](/docs/reference/cli-commands#AnyDeals-insights).
...
When Anydeals cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Anydeals uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Anydeals in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.AnyDeals/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.AnyDeals/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.AnyDeals/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Anydeals process in the same `ANYDEALS_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.AnyDeals/config.yaml`:
...
AnyDeals sessions delete <session_id>
