...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Anydeals Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Anydeals to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.AnyDeals/.env`:
...
All standard Anydeals commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.AnyDeals/logs/gateway.log`
...
4. Consider enabling verbose logging: `ANYDEALS_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Anydeals automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ANYDEALS_LOG_LEVEL=debug AnyDeals gateway
```
...
```bash
AnyDeals chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
