...
sidebar_position: 4
title: "Slack"
description: "Set up Anydeals Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Anydeals Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Anydeals instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Anydeals uses the modern
...
The fastest path is to paste a manifest Anydeals generates for you. It
...
### Option A: From a Anydeals-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   AnyDeals slack manifest --write
   ```
   This writes `~/.AnyDeals/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Anydeals Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Anydeals can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `AnyDeals-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Anydeals configuration issue.
:::
...
Anydeals uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
AnyDeals gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
AnyDeals gateway              # Foreground
AnyDeals gateway install      # Install as a user service
sudo AnyDeals gateway install --system   # Linux only: boot-time system service
```
...
Every Anydeals command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Anydeals command with its description.

Under the hood: Anydeals ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/AnyDeals-agent/blob/main/AnyDeals_cli/commands.py)
...
When Anydeals adds new commands (e.g. after `AnyDeals update`), regenerate
...
3. Paste the new contents of `~/.AnyDeals/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/AnyDeals <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/AnyDeals btw run the tests` — Anydeals routes it the same way as `/btw
run the tests`. Free-form questions also work: `/AnyDeals what's the
...
```bash
AnyDeals slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Anydeals behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Anydeals Agent what time is it?`). In channels, Anydeals replies in a thread attached to that message. |
| **Threads** | If you @mention Anydeals inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.AnyDeals/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Anydeals in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Anydeals will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Anydeals will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Anydeals Agent`).

---

## Multi-Workspace Support

Anydeals can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Anydeals also loads tokens from an **OAuth token file** at:

```
~/.AnyDeals/slack_tokens.json
```
...
- When a message arrives, Anydeals uses the correct workspace-specific client to respond.
...
Anydeals supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Anydeals Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Anydeals Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Anydeals now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Anydeals Agent`)
...
- Tokens should be stored in `~/.AnyDeals/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Anydeals config directory
