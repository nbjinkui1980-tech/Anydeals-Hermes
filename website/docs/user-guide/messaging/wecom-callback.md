...
Connect Anydeals to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Anydeals supports two WeCom integration modes:
...
3. Anydeals decrypts the message, queues it for the agent
...
(Use `AnyDeals gateway start` only after `AnyDeals gateway install` has registered the systemd/launchd service.)
