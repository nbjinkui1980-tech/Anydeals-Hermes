...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Anydeals Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Anydeals Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
Anydeals also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `AnyDeals --tui` — see the [TUI](tui.md) guide.
:::
...
AnyDeals chat --model "anthropic/claude-sonnet-4"

# With a specific provider
AnyDeals chat --provider nous        # Use Nous Portal
AnyDeals chat --provider openrouter  # Force OpenRouter

# With specific toolsets
AnyDeals chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
AnyDeals -s AnyDeals-agent-dev,github-auth
AnyDeals chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
AnyDeals --continue             # Resume the most recent CLI session (-c)
AnyDeals --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
AnyDeals chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
AnyDeals -w                         # Interactive mode in worktree
AnyDeals -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Anydeals CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Anydeals CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
When resuming a previous session (`AnyDeals -c` or `AnyDeals --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Anydeals to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Anydeals supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Anydeals replies |
...
```bash
AnyDeals -s AnyDeals-agent-dev,github-auth
AnyDeals chat -s github-pr-workflow -s github-auth
```

Anydeals loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.AnyDeals/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.AnyDeals/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Anydeals..."
  # Add your own!
```
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Anydeals — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Anydeals is working, Anydeals prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Anydeals to the background — just like any terminal process. The shell prints a confirmation:

```
Anydeals Agent has been suspended. Run `fg` to bring Anydeals Agent back.
```
...
Resume this session with:
  AnyDeals --resume 20260225_143052_a1b2c3
...
```bash
AnyDeals --continue                          # Resume the most recent CLI session
AnyDeals -c                                  # Short form
AnyDeals -c "my project"                     # Resume a named session (latest in lineage)
AnyDeals --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
AnyDeals --resume "refactoring auth"         # Resume by title
AnyDeals -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `AnyDeals sessions rename <id> <title>` from the command line. Use `AnyDeals sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Anydeals's SQLite state database under `~/.AnyDeals/state.db`. The database keeps:
...
Anydeals immediately confirms the task and gives you back the prompt:
