...
it is automatically downloaded from GitHub releases to $ANYDEALS_HOME/bin/tirith.
...
def _get_AnyDeals_home() -> str:
    """Return the Anydeals home directory, respecting ANYDEALS_HOME env var."""
    return str(get_AnyDeals_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_AnyDeals_home(), ".tirith-install-failed")
...
def _AnyDeals_bin_dir() -> str:
    """Return $ANYDEALS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_AnyDeals_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ANYDEALS_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_AnyDeals_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ANYDEALS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ANYDEALS_HOME/bin/tirith
    ...
    AnyDeals_bin = os.path.join(_AnyDeals_bin_dir(), "tirith")
    if os.path.isfile(AnyDeals_bin) and os.access(AnyDeals_bin, os.X_OK):
        _resolved_path = AnyDeals_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return AnyDeals_bin
