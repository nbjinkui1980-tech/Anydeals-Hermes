...
One ``CDPSupervisor`` runs per Anydeals ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "AnyDeals-dialog-bridge.invalid"
...
(() => {
  if (window.__AnyDealsDialogBridgeInstalled) return;
  window.__AnyDealsDialogBridgeInstalled = true;
  const ENDPOINT = "http://AnyDeals-dialog-bridge.invalid/";
