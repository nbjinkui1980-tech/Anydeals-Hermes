...
This module provides the glue:
    - ``AnydealsTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Anydeals Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ANYDEALS_HOME so each profile gets its own OAuth tokens.
    Layout: ``ANYDEALS_HOME/mcp-tokens/``
    """
    try:
        from AnyDeals_constants import get_AnyDeals_home
        base = Path(get_AnyDeals_home())
    except ImportError:
        base = Path(os.environ.get("ANYDEALS_HOME", str(Path.home() / ".AnyDeals")))
...
# ---------------------------------------------------------------------------
# AnydealsTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class AnydealsTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ANYDEALS_HOME/mcp-tokens/<server_name>.json         -- tokens
        ANYDEALS_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ANYDEALS_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Anydeals records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Anydeals.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Anydeals Agent")
    scope = cfg.get("scope")
