...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ANYDEALS_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ANYDEALS_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ANYDEALS_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ANYDEALS_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ANYDEALS_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ANYDEALS_SESSION_PLATFORM via
    ...
    """
    if os.getenv("ANYDEALS_CRON_SESSION"):
        return False
    if os.getenv("ANYDEALS_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ANYDEALS_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ANYDEALS_ENV_PATH = (
    r'(?:~\/\.AnyDeals/|'
    r'(?:\$home|\$\{home\})/\.AnyDeals/|'
    r'(?:\$AnyDeals_home|\$\{AnyDeals_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bAnyDeals\s+gateway\s+(stop|restart)\b', "stop/restart AnyDeals gateway (kills running agents)"),
    (r'\bAnyDeals\s+update\b', "AnyDeals update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart AnyDeals-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart AnyDeals-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(AnyDeals|gateway|cli\.py)\b', "kill AnyDeals/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill AnyDeals` but not
    # `kill -9 $(pgrep -f AnyDeals)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("ANYDEALS_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("ANYDEALS_INTERACTIVE")
    ...
    if is_gateway or os.getenv("ANYDEALS_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("ANYDEALS_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("ANYDEALS_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = os.getenv("ANYDEALS_EXEC_ASK")
