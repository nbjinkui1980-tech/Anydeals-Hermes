"""Profile distributions — shareable, packaged Anydeals profiles via git.

A distribution is a Anydeals profile published as a git repository (or
...
* ``AnyDeals profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``AnyDeals skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``AnyDeals profile``, not a parallel tree):

    AnyDeals profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "AnyDeals_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".AnyDeals_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "AnyDeals-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            AnyDeals_requires=str(data.get("AnyDeals_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.AnyDeals_requires:
            out["AnyDeals_requires"] = self.AnyDeals_requires
        if self.author:
...
def check_AnyDeals_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Anydeals {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``AnyDeals profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Anydeals profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Anydeals distribution."
        )

    # Version check up-front so we fail fast
    check_AnyDeals_requires(manifest.AnyDeals_requires, AnyDeals_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.AnyDeals).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="AnyDeals_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `AnyDeals profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`AnyDeals profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="AnyDeals_dist_update_") as tmp:
        plan = plan_install(
