"""
Configuration management for Anydeals Agent.

Config files are stored in ~/.AnyDeals/ for easy access:
- ~/.AnyDeals/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.AnyDeals/.env         - API keys and secrets

This module provides:
- AnyDeals config          - Show current configuration
- AnyDeals config edit     - Open config in editor
- AnyDeals config set      - Set a specific value
- AnyDeals config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.AnyDeals/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``AnyDeals logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  AnyDeals config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `AnyDeals plugins enable
    # observability/langfuse` or `AnyDeals tools → Langfuse`); credentials gate
...
from AnyDeals_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ANYDEALS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_AnyDeals_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Anydeals is running in package-manager-managed mode.

    Two signals: the ANYDEALS_MANAGED env var (set by the systemd service),
    or a .managed marker file in ANYDEALS_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade AnyDeals-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "AnyDeals update"


def format_managed_message(action: str = "modify this Anydeals installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ANYDEALS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Anydeals installation is managed by NixOS "
            f"(ANYDEALS_MANAGED={env_hint}).\n"
            "Edit services.AnyDeals-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Anydeals installation is managed by Homebrew "
            f"(ANYDEALS_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade AnyDeals-agent"
        )

    return (
        f"Cannot {action}: this Anydeals installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Anydeals."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ANYDEALS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, AnyDeals_bin
    or None if container mode is not active, we're already inside the
    container, or ANYDEALS_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ANYDEALS_DEV") == "1":
        return None
    ...
    container_mode_file = get_AnyDeals_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "AnyDeals-agent")
    exec_user = info.get("exec_user", "AnyDeals")
    AnyDeals_bin = info.get("AnyDeals_bin", "/data/current-package/bin/AnyDeals")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "AnyDeals_bin": AnyDeals_bin,
    }
...
# Re-export from AnyDeals_constants — canonical definition lives there.
from AnyDeals_constants import get_AnyDeals_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the AnyDeals group can
    share state with the gateway service.

    The mode can be overridden via the ANYDEALS_HOME_MODE environment variable
    (e.g. ANYDEALS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ANYDEALS_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ANYDEALS_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Anydeals runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ANYDEALS_CONTAINER") or os.environ.get("ANYDEALS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ANYDEALS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ANYDEALS_HOME if the user doesn't have one yet."""
...
def ensure_AnyDeals_home():
    """Ensure ~/.AnyDeals directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_AnyDeals_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_AnyDeals_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ANYDEALS_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Anydeals-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Anydeals auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Anydeals sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Anydeals
        ...
        # bundled Anydeals image, which drops to the `AnyDeals` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Anydeals sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Anydeals should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``AnyDeals chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.AnyDeals/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: AnyDeals sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Anydeals truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``AnyDeals kanban specify`` (single id or --all). Set a
        ...
        # model; override via `AnyDeals model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `AnyDeals --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `AnyDeals -c` muscle memory.  Default off so existing
        # users aren't surprised.  ANYDEALS_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/AnyDeals`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.AnyDeals/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.AnyDeals/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Anydeals feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Anydeals auto-pauses the goal and
    ...
    # always goes to ~/.AnyDeals/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ANYDEALS_SKILL_DIR} and ${ANYDEALS_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `AnyDeals curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.AnyDeals/skills/ into
        # ~/.AnyDeals/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `AnyDeals curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for AnyDeals-specific overrides; everything else
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ANYDEALS_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.AnyDeals/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ANYDEALS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `AnyDeals doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``AnyDeals_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Anydeals to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ANYDEALS_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `AnyDeals -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     AnyDeals-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.AnyDeals/logs/.
    ...
    # update model picker lists without shipping a AnyDeals-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://AnyDeals-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `AnyDeals model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.AnyDeals/state.db.
    ...
    "sessions": {
        ...
        # default of ``AnyDeals sessions prune``.
        "retention_days": 90,
    ...
    # ``AnyDeals update`` behaviour.
    "updates": {
        # Run a full ``AnyDeals backup``-style zip of ANYDEALS_HOME before every
        # ``AnyDeals update``.  Backups land in ``<ANYDEALS_HOME>/backups/`` and
        # can be restored with ``AnyDeals import <path>``.  Off by default —
        # on large ANYDEALS_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<ANYDEALS_HOME>/lsp/bin/`` on first use.
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'AnyDeals model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Anydeals auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @AnyDeals:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ANYDEALS_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #AnyDeals)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: AnyDeals-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'AnyDeals-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Anydeals API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Anydeals API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Anydeals API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ANYDEALS_TOOL_PROGRESS and ANYDEALS_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "ANYDEALS_TOOL_PROGRESS": {
    ...
    "ANYDEALS_PREFILL_MESSAGES_FILE": {
    ...
    "ANYDEALS_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.AnyDeals.config`` entries, then checks
    ...
    try:
        ...
        # should never break `AnyDeals update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Anydeals versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``AnyDeals_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'AnyDeals setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Anydeals won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'AnyDeals doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ANYDEALS_HOME", "~/.AnyDeals")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ANYDEALS_TOOL_PROGRESS")
            old_mode = get_env_value("ANYDEALS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ANYDEALS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ANYDEALS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ANYDEALS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ANYDEALS_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ANYDEALS_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_AnyDeals_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `AnyDeals plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `AnyDeals curator status` has no
    ...
    #   3. Creates `~/.AnyDeals/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_AnyDeals_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_AnyDeals_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: AnyDeals config set <key> <value>")
    ...
    # metadata.AnyDeals.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _AnyDeals_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.AnyDeals/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.AnyDeals/config.yaml.
    ...
    (which change ``ANYDEALS_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_AnyDeals_home()
        config_path = get_config_path()
        path_key = str(config_path)
...
#   openai-codex (OAuth — AnyDeals auth) — OpenAI Codex
#   nous         (OAuth — AnyDeals auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.AnyDeals/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.AnyDeals/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Anydeals env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.AnyDeals/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.AnyDeals/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.AnyDeals/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.AnyDeals/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Anydeals — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Anydeals vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.AnyDeals/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Anydeals Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  AnyDeals config edit     # Edit config file", Colors.DIM))
    print(color("  AnyDeals config set <key> <value>", Colors.DIM))
    print(color("  AnyDeals setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: AnyDeals config set <key> <value>")
            print()
            print("Examples:")
            print("  AnyDeals config set model anthropic/claude-sonnet-4")
            print("  AnyDeals config set terminal.backend docker")
            print("  AnyDeals config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'AnyDeals config migrate' to add them")
        ...
        print("  AnyDeals config           Show current configuration")
        print("  AnyDeals config edit      Open config in editor")
        print("  AnyDeals config set <key> <value>   Set a config value")
        print("  AnyDeals config check     Check for missing/outdated config")
        print("  AnyDeals config migrate   Update config with new options")
        print("  AnyDeals config path      Show config file path")
        print("  AnyDeals config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``AnyDeals config`` UI so users
