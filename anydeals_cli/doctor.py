"""
Doctor command for AnyDeals CLI.

Diagnoses issues with Anydeals Agent setup.
"""
...
from AnyDeals_cli.config import get_project_root, get_AnyDeals_home, get_env_path
...
_DHH = display_AnyDeals_home()  # user-facing display path (e.g. ~/.AnyDeals or ~/.AnyDeals/profiles/coder)

# Load environment variables from ~/.AnyDeals/.env so API key checks work
_env_path = get_env_path()
load_AnyDeals_dotenv(AnyDeals_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from AnyDeals_cli.colors import Colors, color
from AnyDeals_cli.models import _ANYDEALS_USER_AGENT
from AnyDeals_cli.vercel_auth import describe_vercel_auth
from AnyDeals_constants import OPENROUTER_MODELS_URL
...
from AnyDeals_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.AnyDeals/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("ANYDEALS_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ANYDEALS_KANBAN_TASK"):
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from AnyDeals_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `AnyDeals`.
    os.environ.setdefault("ANYDEALS_INTERACTIVE", "1")

    # Handle `AnyDeals doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.AnyDeals/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Anydeals Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`AnyDeals doctor --ack {hit.advisory.id}`."
                )
    ...
    # Check ~/.AnyDeals/.env (primary location for user config)
    env_path = ANYDEALS_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'AnyDeals setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'AnyDeals setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'AnyDeals setup' to create one")
                issues.append("Run 'AnyDeals setup' to create .env")
    
    # Check ~/.AnyDeals/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ANYDEALS_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from AnyDeals_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from AnyDeals_cli.providers import (
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'AnyDeals config set model.provider <valid_provider>'"
                    )
            ...
            if runtime_provider and runtime_provider not in {"auto", "custom", "openrouter"}:
                try:
                    from AnyDeals_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.AnyDeals/.env or run 'AnyDeals setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'AnyDeals setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'AnyDeals config set model.provider <name>'"
                            )
                except Exception:
                    pass
    ...
    if config_path.exists():
        try:
            from AnyDeals_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'AnyDeals setup' to migrate config")
                else:
                    issues.append("Run 'AnyDeals doctor --fix' or 'AnyDeals setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'AnyDeals doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses Anydeals' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `AnyDeals auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    AnyDeals_home = ANYDEALS_HOME
    if AnyDeals_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        AnyDeals_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = AnyDeals_home / subdir_name
    ...
    soul_path = AnyDeals_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Anydeals a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Anydeals Agent Persona\n\n"
                "<!-- Edit this file to customize how Anydeals communicates. -->\n\n"
                "You are Anydeals, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = AnyDeals_home / "memories"
    ...
    state_db_path = AnyDeals_home / "state.db"
    ...
    wal_path = AnyDeals_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'AnyDeals doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (AnyDeals bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "AnyDeals"
        ...
        _cmd_link = _cmd_link_dir / "AnyDeals"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(AnyDeals not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/AnyDeals → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/AnyDeals points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/AnyDeals → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/AnyDeals — run 'AnyDeals doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/AnyDeals exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/AnyDeals not found",
                    "(AnyDeals command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/AnyDeals → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/AnyDeals symlink — run 'AnyDeals doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'AnyDeals-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'AnyDeals-agent[vercel]'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `AnyDeals doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'AnyDeals config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _ANYDEALS_USER_AGENT,
            }
    ...
    # never the legitimate source for `AnyDeals doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'AnyDeals setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = ANYDEALS_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: AnyDeals skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ANYDEALS_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: AnyDeals memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: AnyDeals memory setup")
                issues.append("No Honcho API key — run 'AnyDeals memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run AnyDeals memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: AnyDeals memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: AnyDeals memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from AnyDeals_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "AnyDeals -p" in content:
                            _m = _re.search(r"AnyDeals -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'AnyDeals doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
