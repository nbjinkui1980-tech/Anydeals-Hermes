"""
AnyDeals fallback — manage the fallback provider chain.
...
Subcommands:
  AnyDeals fallback [list]   Show the current fallback chain (default when no subcommand)
  AnyDeals fallback add      Pick provider + model via the same picker as `AnyDeals model`,
                           then append the selection to the chain
  AnyDeals fallback remove   Pick an entry to delete from the chain
  AnyDeals fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.AnyDeals/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from AnyDeals_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `AnyDeals model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  AnyDeals fallback add")
        print()
        return
    ...
    print("  Docs: https://AnyDeals-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `AnyDeals model`, then append the selection to the chain."""
    from AnyDeals_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `AnyDeals model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `AnyDeals fallback list` to view, or `AnyDeals fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``AnyDeals fallback [subcommand]``."""
