...
Usage:
    AnyDeals pairing list              # Show all pending + approved users
    AnyDeals pairing approve <platform> <code>  # Approve a pairing code
    AnyDeals pairing revoke <platform> <user_id> # Revoke user access
    AnyDeals pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle AnyDeals pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: AnyDeals pairing {list|approve|revoke|clear-pending}")
        print("Run 'AnyDeals pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.AnyDeals/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'AnyDeals pairing list' to see pending codes.\n")
