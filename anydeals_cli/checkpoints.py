"""`AnyDeals checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.AnyDeals/checkpoints/``.  Actions:

    AnyDeals checkpoints               # same as `status`
    AnyDeals checkpoints status        # total size, project count, breakdown
    AnyDeals checkpoints list          # per-project checkpoint counts + workdir
    AnyDeals checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    AnyDeals checkpoints clear [-f]    # nuke the entire base (asks first)
    AnyDeals checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    AnyDeals checkpoints
    AnyDeals checkpoints prune --retention-days 3 --max-size-mb 200
    AnyDeals checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: AnyDeals checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``AnyDeals checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `AnyDeals checkpoints` → status
