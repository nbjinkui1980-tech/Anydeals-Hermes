#!/usr/bin/env python3
"""
Anydeals CLI - Main entry point.

Usage:
    AnyDeals                     # Interactive chat (default)
    AnyDeals chat                # Interactive chat
    AnyDeals gateway             # Run gateway in foreground
    AnyDeals gateway start       # Start gateway as service
    AnyDeals gateway stop        # Stop gateway service
    AnyDeals gateway status      # Show gateway status
    AnyDeals gateway install     # Install gateway service
    AnyDeals gateway uninstall   # Uninstall gateway service
    AnyDeals setup               # Interactive setup wizard
    AnyDeals logout              # Clear stored authentication
    AnyDeals status              # Show status of all components
    AnyDeals cron                # Manage cron jobs
    AnyDeals cron list           # List cron jobs
    AnyDeals cron status         # Check if cron scheduler is running
    AnyDeals doctor              # Check configuration and dependencies
    AnyDeals honcho setup                    # Configure Honcho AI memory integration
    AnyDeals honcho status                   # Show Honcho config and connection status
    AnyDeals honcho sessions                 # List directory → session name mappings
    AnyDeals honcho map <name>               # Map current directory to a session name
    AnyDeals honcho peer                     # Show peer names and dialectic settings
    AnyDeals honcho peer --user NAME         # Set user peer name
    AnyDeals honcho peer --ai NAME           # Set AI peer name
    AnyDeals honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    AnyDeals honcho mode                     # Show current memory mode
    AnyDeals honcho mode [hybrid|honcho|local]  # Set memory mode
    AnyDeals honcho tokens                   # Show token budget settings
    AnyDeals honcho tokens --context N       # Set session.context() token cap
    AnyDeals honcho tokens --dialectic N     # Set dialectic result char cap
    AnyDeals honcho identity                 # Show AI peer identity representation
    AnyDeals honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    AnyDeals honcho migrate                  # Step-by-step migration guide: OpenClaw native → Anydeals + Honcho
    AnyDeals version             Show version
    AnyDeals update              Update to latest version
    AnyDeals uninstall           Uninstall Anydeals Agent
    AnyDeals acp                 Run as an ACP server for editor integration
    AnyDeals sessions browse     Interactive session picker with search

    AnyDeals claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: AnyDeals_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``AnyDeals_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``AnyDeals update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``AnyDeals_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, AnyDeals crashes on import and the user can't run
# ``AnyDeals update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ANYDEALS_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (AnyDeals tools, AnyDeals setup, AnyDeals model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'AnyDeals {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any AnyDeals module import.
#
# Many modules cache ANYDEALS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ANYDEALS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.AnyDeals/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ANYDEALS_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors AnyDeals_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ANYDEALS_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.AnyDeals/profiles/coder or
    # /opt/data/profiles/coder).  If ANYDEALS_HOME points to the AnyDeals root
    # instead (e.g. systemd hardcodes ANYDEALS_HOME=/root/.AnyDeals), we must
    # still read active_profile — the user may have switched profiles via
    # `AnyDeals profile use` and the gateway should honour that choice.
    # See issue #22502.
    AnyDeals_home_env = os.environ.get("ANYDEALS_HOME", "")
    if profile_name is None and AnyDeals_home_env:
        if Path(AnyDeals_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the AnyDeals root
    if profile_name is None:
        try:
            from AnyDeals_constants import get_default_AnyDeals_root

            active_path = get_default_AnyDeals_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ANYDEALS_HOME
    if profile_name is not None:
        try:
            from AnyDeals_cli.profiles import resolve_profile_env

            AnyDeals_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent AnyDeals from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ANYDEALS_HOME"] = AnyDeals_home
...
from AnyDeals_cli.config import get_AnyDeals_home
from AnyDeals_cli.env_loader import load_AnyDeals_dotenv

load_AnyDeals_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ANYDEALS_REDACT_SECRETS env
# var BEFORE AnyDeals_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "ANYDEALS_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_AnyDeals_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["ANYDEALS_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `AnyDeals` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from AnyDeals_logging import setup_logging as _setup_logging
...
try:
    from AnyDeals_cli.config import load_config as _load_config_early
    from AnyDeals_constants import apply_ipv4_preference as _apply_ipv4
...
from AnyDeals_cli import __version__, __release_date__
from AnyDeals_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from AnyDeals_cli.config import get_env_path, get_AnyDeals_home, load_config
    from AnyDeals_cli.auth import get_auth_status

    # Determine whether Anydeals itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_AnyDeals_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_AnyDeals_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Anydeals has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Anydeals to use their tokens.
    if _has_AnyDeals_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, AnyDeals_bin
        cli_args: the original CLI arguments (everything after 'AnyDeals')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    AnyDeals_bin = container_info["AnyDeals_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo AnyDeals {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo AnyDeals {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  AnyDeals --tui --resume {target}")
    if title:
        print(f'  AnyDeals --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @AnyDeals/ink is missing or node_modules is behind package-lock.json.
...
def _ensure_tui_node() -> None:
    ...
    ``ANYDEALS_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ANYDEALS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    AnyDeals_home = os.environ.get("ANYDEALS_HOME") or str(Path.home() / ".AnyDeals")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ANYDEALS_HOME": AnyDeals_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(AnyDeals_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ANYDEALS_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ANYDEALS_NODE")
    ...
    ext_dir = os.environ.get("ANYDEALS_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ANYDEALS_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ANYDEALS_TUI_DIR (e.g. `unset ANYDEALS_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("ANYDEALS_QUIET"):
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from AnyDeals_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="AnyDeals-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ANYDEALS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ANYDEALS_PYTHON_SRC_ROOT"] = os.environ.get(
        "ANYDEALS_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ANYDEALS_PYTHON", sys.executable)
    env.setdefault("ANYDEALS_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ANYDEALS_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ANYDEALS_MODEL"] = model
        env["ANYDEALS_INFERENCE_MODEL"] = model
    if provider:
        env["ANYDEALS_TUI_PROVIDER"] = provider
        env["ANYDEALS_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ANYDEALS_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ANYDEALS_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ANYDEALS_TUI_SKILLS"] = value
    if query:
        env["ANYDEALS_TUI_QUERY"] = query
    if image:
        env["ANYDEALS_TUI_IMAGE"] = image
    if checkpoints:
        env["ANYDEALS_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ANYDEALS_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ANYDEALS_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ANYDEALS_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ANYDEALS_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ANYDEALS_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["ANYDEALS_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ANYDEALS_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``AnyDeals kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``AnyDeals kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ANYDEALS_KANBAN_BOARD"):
        return
    try:
        from AnyDeals_cli.kanban_db import get_current_board

        os.environ["ANYDEALS_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("ANYDEALS_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'AnyDeals sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Anydeals isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  AnyDeals setup")
        print()

        from AnyDeals_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'AnyDeals setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from AnyDeals_cli.banner import prefetch_update_check
    ...
    # user's ~/.AnyDeals/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ANYDEALS_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ANYDEALS_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ANYDEALS_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from AnyDeals_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from AnyDeals_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Anydeals?")
        print()
    ...
    session_dir = get_AnyDeals_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: AnyDeals gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  AnyDeals gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Anydeals Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  AnyDeals gateway")
        ...
        print()
        print("  Or install as a service: AnyDeals gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'AnyDeals whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from AnyDeals_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``AnyDeals model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from AnyDeals_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from AnyDeals_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ANYDEALS_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'AnyDeals model' for "
                "available providers, or run 'AnyDeals doctor' to diagnose config "
                "issues."
            )
    ...
    from AnyDeals_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from AnyDeals_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.AnyDeals/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.AnyDeals/.env if the active provider is not 'custom'.
    ...
    """
    from AnyDeals_cli.config import get_env_value, save_env_value, load_config
...
#
# Anydeals uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `AnyDeals model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `AnyDeals model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Anydeals only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``AnyDeals model`` flow, then route aux tasks to them here.
    """
    from AnyDeals_cli.config import load_config
    from AnyDeals_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from AnyDeals_cli.auth import _prompt_model_selection
    from AnyDeals_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from AnyDeals_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from AnyDeals_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from AnyDeals_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from AnyDeals_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from AnyDeals_cli.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from AnyDeals_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from AnyDeals_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `AnyDeals auth` stores device_code tokens),
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from AnyDeals_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from AnyDeals_cli.models import _PROVIDER_MODELS
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.AnyDeals/config.yaml.
    """
    from AnyDeals_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from AnyDeals_cli.auth import _save_model_choice, deactivate_provider
    from AnyDeals_cli.config import get_env_value, load_config, save_config
    ...
    from AnyDeals_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `AnyDeals model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from AnyDeals_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from AnyDeals_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from AnyDeals_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Anydeals will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from AnyDeals_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from AnyDeals_cli.config import save_env_value, load_config, save_config
    from AnyDeals_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from AnyDeals_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from AnyDeals_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Anydeals turns to `copilot --acp`.")
    print("  Anydeals currently starts its own ACP subprocess for each request.")
    print("  Anydeals uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ANYDEALS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``AnyDeals setup`` / ``AnyDeals model``.
    ...
    recover from a malformed paste without editing ``~/.AnyDeals/.env`` by hand.
    ...
    from AnyDeals_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from AnyDeals_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `AnyDeals setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so AnyDeals knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Anydeals typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Anydeals, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from AnyDeals_cli.auth import AuthError
        from AnyDeals_cli.models import fetch_lmstudio_models
        ...
        from AnyDeals_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Anydeals will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from AnyDeals_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'AnyDeals model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Anydeals CLI with a provider."""
    from AnyDeals_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from AnyDeals_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from AnyDeals_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from AnyDeals_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from AnyDeals_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from AnyDeals_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``AnyDeals slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: AnyDeals slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `AnyDeals slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from AnyDeals_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from AnyDeals_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from AnyDeals_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from AnyDeals_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from AnyDeals_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from AnyDeals_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from AnyDeals_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Anydeals home directory to a zip file."""
    if getattr(args, "quick", False):
        from AnyDeals_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from AnyDeals_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Anydeals backup from a zip file."""
    from AnyDeals_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Anydeals Agent v{__version__} ({__release_date__})")
    ...
    try:
        from AnyDeals_cli.banner import check_for_updates
        from AnyDeals_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Anydeals Agent."""
    _require_tty("uninstall")
    from AnyDeals_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``AnyDeals update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``AnyDeals_cli/web_dist/`` (per vite.config.ts
    outDir: "../AnyDeals_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "AnyDeals_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``AnyDeals update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``AnyDeals web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (AnyDeals web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``AnyDeals dashboard`` processes other than ourselves.

    ``AnyDeals dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``AnyDeals update`` replaces files on disk, the running
    ...
    patterns = [
        "AnyDeals dashboard",
        "AnyDeals_cli.main dashboard",
        "AnyDeals_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "AnyDeals.*dashboard"`) keeps us consistent
            # with `AnyDeals_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `AnyDeals update`.
    ...
    print("  Preview now:  AnyDeals curator run --dry-run")
    print("  Pause it:     AnyDeals curator pause")
    print(
        "  Docs:         https://AnyDeals-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``AnyDeals update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``AnyDeals update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: AnyDeals curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``AnyDeals dashboard`` processes.

    Called at the end of ``AnyDeals update`` (default ``reason``) and also
    from ``AnyDeals dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    AnyDeals dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Anydeals Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/AnyDeals-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="AnyDeals-update-")
        zip_path = os.path.join(tmp_dir, f"AnyDeals-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to AnyDeals-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"AnyDeals-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "AnyDeals-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Anydeals behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes AnyDeals completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Anydeals couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Anydeals couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Anydeals behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `AnyDeals update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/AnyDeals-agent.git",
    "git@github.com:NousResearch/AnyDeals-agent.git",
    "https://github.com/NousResearch/AnyDeals-agent",
    "git@github.com:NousResearch/AnyDeals-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/AnyDeals-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_AnyDeals_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_AnyDeals_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Anydeals repository.")
        print("  This means you may miss updates from NousResearch/AnyDeals-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/AnyDeals-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/AnyDeals-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``AnyDeals update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from AnyDeals_constants import get_default_AnyDeals_root

    default_home = get_default_AnyDeals_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``AnyDeals update`` is still progressing even if pip/uv itself is silent.
    """
...
def _AnyDeals_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "AnyDeals.exe",
        scripts_dir / "AnyDeals-gateway.exe",
    ]


def _quarantine_running_AnyDeals_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``AnyDeals.exe``.
    ...
    when ``AnyDeals update`` runs, ``AnyDeals.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``AnyDeals.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next AnyDeals invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _AnyDeals_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_AnyDeals_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``AnyDeals.exe.old.*`` left by prior updates.

    Called early on every AnyDeals invocation. The .old files are unlocked once
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``AnyDeals.exe`` / ``AnyDeals-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_AnyDeals_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_AnyDeals_exe(scripts_dir)
        try:
...
class _UpdateOutputStream:
    """Stream wrapper used during ``AnyDeals update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.AnyDeals/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``AnyDeals update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``AnyDeals update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.AnyDeals/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``AnyDeals update --gateway``) the update is already
    ...
    try:
        ...
        # AnyDeals_cli.config.get_AnyDeals_home to simulate setup failure.
        from AnyDeals_cli.config import get_AnyDeals_home as _get_AnyDeals_home

        logs_dir = _get_AnyDeals_home() / "logs"
...
def _cmd_update_check():
    """Implement ``AnyDeals update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``AnyDeals update`` without requiring a reinstall.  The
    ...
    ``AnyDeals`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/AnyDeals.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v AnyDeals'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/AnyDeals, code at /usr/local/lib/AnyDeals-agent).
    fhs_link = Path("/usr/local/bin/AnyDeals")
    ...
    path_comment = (
        "# Anydeals Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ANYDEALS_HOME before running the update.
    ...
    ANYDEALS_HOME directories.  The ``--backup`` flag on ``AnyDeals update``
    ...
    try:
        from AnyDeals_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_AnyDeals_home so the user sees ~/.AnyDeals/...
    try:
        from AnyDeals_constants import get_AnyDeals_home, display_AnyDeals_home

        home = get_AnyDeals_home()
        try:
            display_path = f"{display_AnyDeals_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  AnyDeals import {out_path}")
...
def cmd_update(args):
    """Update Anydeals Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from AnyDeals_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Anydeals Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_AnyDeals_home added to AnyDeals_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload AnyDeals_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_AnyDeals_home() added since the last release.
        try:
            import importlib
            import AnyDeals_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ANYDEALS_HOME so
        # it is not affected by sync_skills()'s module-level ANYDEALS_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ANYDEALS_HOME env var points at the default or a named profile.
        try:
            from AnyDeals_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: AnyDeals config migrate")
            else:
                print()
                print("Skipped. Run 'AnyDeals config migrate' later to configure.")
        else:
        ...
        # check `AnyDeals curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``AnyDeals update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from AnyDeals_cli.tools_config import install_cua_driver
        ...
        # When running as ``AnyDeals update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_AnyDeals_home() / ".update_exit_code"
            try:
        ...
        try:
            from AnyDeals_cli.gateway import (
                is_macos,
            ...
            try:
                from AnyDeals_constants import (
            ...
            # Discover all AnyDeals-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. AnyDeals-gateway.service or AnyDeals-gateway-coder.service
                            ...
                            # path in `AnyDeals gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: AnyDeals gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: AnyDeals -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Anydeals gateway unit files are still installed.
        # When both AnyDeals.service (from a pre-rename install) and the
        # current AnyDeals-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `AnyDeals update` surfaces the issue until the user migrates.
        try:
            from AnyDeals_cli.gateway import (
                has_legacy_AnyDeals_units,
                _find_legacy_AnyDeals_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_AnyDeals_units():
                print()
                print("⚠ Legacy Anydeals gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_AnyDeals_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (AnyDeals.service) fight the current")
                print("  AnyDeals-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    AnyDeals gateway migrate-legacy")
                print()
        ...
        print("  AnyDeals model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``AnyDeals -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from AnyDeals_constants import display_AnyDeals_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `AnyDeals profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_AnyDeals_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → AnyDeals -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.AnyDeals)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  AnyDeals profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     AnyDeals -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `AnyDeals profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from AnyDeals_cli.profiles import profile_exists
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec AnyDeals -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from AnyDeals_cli.profiles import rename_profile
        ...
        from AnyDeals_cli.profiles import export_profile
        ...
        from AnyDeals_cli.profiles import import_profile
        ...
        import tempfile
        from AnyDeals_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="AnyDeals_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  AnyDeals -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      AnyDeals -p {plan.manifest.name} chat")
        ...
        from AnyDeals_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `AnyDeals profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"AnyDeals -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from AnyDeals_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("AnyDeals_requires"):
            print(f"Requires:     Anydeals {data['AnyDeals_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from AnyDeals_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.AnyDeals_requires:
        print(f"  Requires: Anydeals {mf.AnyDeals_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``AnyDeals dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``AnyDeals dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No AnyDeals dashboard processes running.")
        return 0

    print(f"{len(pids)} AnyDeals dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No AnyDeals dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `AnyDeals update`.
    ...
    if "ANYDEALS_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ANYDEALS_WEB_DIST"])
            if "ANYDEALS_WEB_DIST" in os.environ
            else PROJECT_ROOT / "AnyDeals_cli" / "web_dist"
        )
    ...
    from AnyDeals_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("ANYDEALS_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from AnyDeals_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Anydeals log files."""
    from AnyDeals_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from AnyDeals_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``AnyDeals -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``AnyDeals_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``AnyDeals -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``AnyDeals`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for AnyDeals CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from AnyDeals_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``AnyDeals.exe.old.*`` quarantine files left by previous
    # ``AnyDeals update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_AnyDeals_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from AnyDeals_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: AnyDeals-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from AnyDeals_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://AnyDeals-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `AnyDeals model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy AnyDeals.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Anydeals gateway unit files "
            "(e.g. AnyDeals.service) left over from older installs. Profile "
            "units (AnyDeals-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Anydeals Agent with an interactive wizard. "
        "Run a specific section: AnyDeals setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'AnyDeals setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Anydeals.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $ANYDEALS_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Anydeals")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Anydeals CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: AnyDeals-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Anydeals with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set ANYDEALS_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Anydeals Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.AnyDeals/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.AnyDeals/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from AnyDeals_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.AnyDeals/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.AnyDeals/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Anydeals Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `AnyDeals "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Anydeals setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Anydeals Agent. Use 'AnyDeals debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    AnyDeals debug share              Upload debug report and print URL
    AnyDeals debug share --lines 500  Include more log lines
    AnyDeals debug share --expire 30  Keep paste for 30 days
    AnyDeals debug share --local      Print report locally (no upload)
    AnyDeals debug share --no-redact  Disable upload-time secret redaction
    AnyDeals debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'AnyDeals debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Anydeals home directory to a zip file",
        description="Create a zip archive of your entire Anydeals configuration, "
        "skills, sessions, and data (excludes the AnyDeals-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/AnyDeals-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.AnyDeals/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo AnyDeals uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from AnyDeals_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Anydeals backup from a zip file",
        description="Extract a previously created Anydeals backup into your "
        "Anydeals home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Anydeals Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from AnyDeals_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.AnyDeals/skills/.bundled_manifest) "
            "so future 'AnyDeals update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from AnyDeals_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from AnyDeals_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/AnyDeals-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `AnyDeals plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from AnyDeals_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``AnyDeals --help``, ``AnyDeals version``, ``AnyDeals logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from AnyDeals_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from AnyDeals_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_AnyDeals_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_AnyDeals_home()}/memories/\n")
        else:
            from AnyDeals_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'AnyDeals tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # AnyDeals tools list [--platform cli]
    ...
    # AnyDeals tools disable <name...> [--platform cli]
    ...
    # AnyDeals tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from AnyDeals_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from AnyDeals_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `AnyDeals computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `AnyDeals tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: AnyDeals computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: AnyDeals computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Anydeals as an MCP server",
        description=(
            "Manage MCP server connections and run Anydeals as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'AnyDeals mcp add' to connect to a new server, or\n"
            "'AnyDeals mcp serve' to expose Anydeals conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Anydeals as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `AnyDeals mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from AnyDeals_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_AnyDeals_home() / "sessions"
            ...
            # Launch AnyDeals --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from AnyDeals_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Anydeals",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Anydeals",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.AnyDeals/ (by default a "
        "single restore-point archive is written to ~/.AnyDeals/backups/ "
        "before apply; restorable with 'AnyDeals import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from AnyDeals_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Anydeals Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'AnyDeals config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Anydeals Agent",
        description="Remove Anydeals Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Anydeals Agent as an ACP (Agent Client Protocol) server",
        description="Start Anydeals Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Anydeals Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Anydeals instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `AnyDeals update` skill sync)",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Anydeals profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Anydeals Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `AnyDeals --tui` via PTY/WebSocket). "
            "Alternatively set ANYDEALS_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `AnyDeals dashboard` cmdlines and SIGTERM them directly — the same
    # path `AnyDeals update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running AnyDeals dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running AnyDeals dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Anydeals log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    AnyDeals logs                    Show last 50 lines of agent.log
    AnyDeals logs -f                 Follow agent.log in real time
    AnyDeals logs errors             Show last 50 lines of errors.log
    AnyDeals logs gateway -n 100     Show last 100 lines of gateway.log
    AnyDeals logs --level WARNING    Only show WARNING and above
    AnyDeals logs --session abc123   Filter by session ID
    AnyDeals logs --component tools  Only show tool-related lines
    AnyDeals logs --since 1h         Lines from the last hour
    AnyDeals logs --since 30m -f     Follow, starting from 30 min ago
    AnyDeals logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``AnyDeals -c Pokemon Agent Dev`` → ``AnyDeals -c 'Pokemon Agent Dev'``
    ...
    from AnyDeals_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'AnyDeals -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (AnyDeals hooks list, cron
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from AnyDeals_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from AnyDeals_cli.oneshot import run_oneshot
