...
on Japanese locales, etc.).  Anydeals's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``AnyDeals_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Anydeals is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ANYDEALS_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ANYDEALS_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Anydeals-managed Git install directories so
    ...
    # already-running shells don't see the change — which means AnyDeals
    ...
    # install without Anydeals-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Anydeals's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Anydeals's
    ``AnyDeals config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Anydeals (User env var in Windows Settings, or
    export in a PowerShell profile) and Anydeals picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Anydeals-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\AnyDeals\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``AnyDeals`` from right after install) retain their old PATH.

    Any subprocess Anydeals spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Anydeals-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "AnyDeals", "git", "cmd"),
        os.path.join(local_appdata, "AnyDeals", "git", "bin"),
        os.path.join(local_appdata, "AnyDeals", "git", "usr", "bin"),
        # Anydeals venv Scripts directory — host of the AnyDeals.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes AnyDeals, but harmless to include.
        os.path.join(local_appdata, "AnyDeals", "AnyDeals-agent", "venv", "Scripts"),
