...
``<root>`` is the **shared Anydeals root** (the parent of any active
...
``AnyDeals -p <profile>`` joins the same board as the dispatcher that
...
the profile's Anydeals home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``ANYDEALS_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``ANYDEALS_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``AnyDeals kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.AnyDeals``. In Docker / custom
deployments where ``ANYDEALS_HOME`` points outside ``~/.AnyDeals`` (e.g.
``/opt/AnyDeals``), ``<root>`` is ``ANYDEALS_HOME``. Legacy env-var
overrides still work:

* ``ANYDEALS_KANBAN_DB`` — pin the database file path directly.
* ``ANYDEALS_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``ANYDEALS_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``ANYDEALS_KANBAN_DB``,
``ANYDEALS_KANBAN_WORKSPACES_ROOT``, and ``ANYDEALS_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/AnyDeals-kanban-v1-spec.pdf`` for the full
...
# enough that kebab-case names like ``atm10-server`` or ``AnyDeals-agent``
...
def kanban_home() -> Path:
    """Return the shared Anydeals root that anchors the kanban board.

    Resolution order:

    1. ``ANYDEALS_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_AnyDeals_root()``, which already returns ``<root>``
       when ``ANYDEALS_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ANYDEALS_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ANYDEALS_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("ANYDEALS_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from AnyDeals_constants import get_default_AnyDeals_root
    return get_default_AnyDeals_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``AnyDeals kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``ANYDEALS_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``AnyDeals kanban boards
    ...
    """
    env = os.environ.get("ANYDEALS_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``AnyDeals kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``ANYDEALS_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("ANYDEALS_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``ANYDEALS_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("ANYDEALS_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``AnyDeals kanban log`` unambiguous even when multiple
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``ANYDEALS_KANBAN_DB`` env → ``ANYDEALS_KANBAN_BOARD`` env →
    ...
    # See AnyDeals_state._WAL_INCOMPAT_MARKERS for detection logic.
    from AnyDeals_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``AnyDeals kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from AnyDeals_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``AnyDeals --skills ...`` alongside the built-in
    ...
    # `AnyDeals --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `AnyDeals kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``AnyDeals kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``AnyDeals kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``AnyDeals kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Anydeals
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``AnyDeals kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Anydeals profile.
    ...
    if not rows:
        return False
    try:
        from AnyDeals_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Anydeals profile.
        # `_default_spawn` invokes ``AnyDeals -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Anydeals
...
def _resolve_AnyDeals_argv() -> list[str]:
    """Resolve the ``AnyDeals`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``shutil.which("AnyDeals")`` — the console-script shim, the same form
       that shows up in ``ps`` output and existing logs. Preferred so live
       systems' diagnostics stay familiar.
    2. ``sys.executable -m AnyDeals_cli.main`` — fallback for setups where
       Anydeals is launched from a venv and the ``AnyDeals`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_AnyDeals_bin`` for the same reason. Kept
    local (not imported from gateway) because ``AnyDeals_cli`` sits below
    ...
    if AnyDeals_bin:
        return [AnyDeals_bin]
    # Fallback to the module form. ``AnyDeals_cli.main`` is the actual
    # console-script target declared in pyproject.toml, NOT a top-level
    # ``AnyDeals`` package — there is no ``AnyDeals`` package to import.
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``AnyDeals -p <profile> chat -q ...`` subprocess.
    ...
    ``ANYDEALS_KANBAN_DB`` / ``ANYDEALS_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject ANYDEALS_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `AnyDeals -p <name>` the
    # _apply_profile_override() runs *before* AnyDeals_constants is imported.
    # If ANYDEALS_HOME is absent from the child's env, get_AnyDeals_home() falls
    # back to Path.home() / ".AnyDeals" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from AnyDeals_cli.profiles import resolve_profile_env
    try:
        env["ANYDEALS_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via ANYDEALS_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ANYDEALS_HOME never had profiles created.
        pass
    if task.tenant:
        env["ANYDEALS_TENANT"] = task.tenant
    env["ANYDEALS_KANBAN_TASK"] = task.id
    env["ANYDEALS_KANBAN_WORKSPACE"] = workspace
    if task.current_run_id is not None:
        env["ANYDEALS_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ANYDEALS_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Pin the shared board + workspaces root the dispatcher resolved, so
    # that even when the worker activates a profile (`AnyDeals -p <name>`
    # rewrites ANYDEALS_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_AnyDeals_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["ANYDEALS_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["ANYDEALS_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["ANYDEALS_KANBAN_BOARD"] = resolved_board
    # ANYDEALS_PROFILE is the author the kanban_comment tool defaults to.
    # `AnyDeals -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["ANYDEALS_PROFILE"] = profile_arg

    cmd = [
        *_resolve_AnyDeals_argv(),
        "-p", profile_arg,
    ...
    # `AnyDeals kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`AnyDeals` executable not found on PATH. "
            "Install Anydeals Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``AnyDeals kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled ANYDEALS_PROFILE values like "AnyDeals-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Anydeals root exists

    Reads profile paths directly so this module has no import dependency on
    ``AnyDeals_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from AnyDeals_constants import get_default_AnyDeals_root
        default_root = get_default_AnyDeals_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``AnyDeals kanban assignees`` for the terminal.
