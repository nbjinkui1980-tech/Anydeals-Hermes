"""
Anydeals Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.AnyDeals/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Anydeals PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing AnyDeals-agent or AnyDeals PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Anydeals Agent" comment and following line
                if '# Anydeals Agent' in line or '# AnyDeals-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('AnyDeals' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing AnyDeals
                if 'AnyDeals' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the AnyDeals wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "AnyDeals",
        Path("/usr/local/bin/AnyDeals"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains AnyDeals_cli reference)
                content = wrapper.read_text()
                if 'AnyDeals_cli' in content or 'AnyDeals-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``AnyDeals gateway run`` processes
    ...
    try:
        from AnyDeals_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from AnyDeals_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ANYDEALS_HOME`` and ``ANYDEALS_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\AnyDeals\git\cmd``, ``%LOCALAPPDATA%\AnyDeals\git\bin``,
#      ``%LOCALAPPDATA%\AnyDeals\git\usr\bin``, ``%LOCALAPPDATA%\AnyDeals\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\AnyDeals\git\`` and Node to
#      ``%LOCALAPPDATA%\AnyDeals\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``AnyDeals dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\AnyDeals\gateway-service\`` and sometimes
...
def _AnyDeals_path_markers(AnyDeals_home: Path) -> list[str]:
    """Path-entry substrings that identify Anydeals-owned User-PATH entries."""
    root = str(AnyDeals_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare AnyDeals-agent install dir.
    markers = [root + "\\AnyDeals-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ANYDEALS_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "AnyDeals".  We don't want to catch
    # unrelated entries like "cAnyDeals-foo" or "ephermeral", so we look for
    # backslash-AnyDeals as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(AnyDeals_home: Path) -> list[str]:
    """Strip Anydeals-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _AnyDeals_path_markers(AnyDeals_home)
            kept: list[str] = []
...
def remove_AnyDeals_env_vars_windows() -> list[str]:
    """Delete ANYDEALS_HOME and ANYDEALS_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ANYDEALS_HOME", "ANYDEALS_GIT_BASH_PATH"):
                try:
...
def _is_default_AnyDeals_home(AnyDeals_home: Path) -> bool:
    """Return True when ``AnyDeals_home`` points at the default (non-profile) root."""
    try:
        from AnyDeals_constants import get_default_AnyDeals_root
        return AnyDeals_home.resolve() == get_default_AnyDeals_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ANYDEALS_HOME directory.

    We shell out to ``AnyDeals -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ANYDEALS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m AnyDeals_cli.main` so we don't depend on a `AnyDeals`
    #    wrapper that may be half-removed mid-uninstall.
    AnyDeals_invocation = [_sys.executable, "-m", "AnyDeals_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                AnyDeals_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ANYDEALS_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.AnyDeals/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.AnyDeals/ for future reinstall
    """
    project_root = get_project_root()
    AnyDeals_home = get_AnyDeals_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ANYDEALS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_AnyDeals_home(AnyDeals_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Anydeals Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {AnyDeals_home / 'config.yaml'}")
    print(f"  Secrets: {AnyDeals_home / '.env'}")
    print(f"  Data:    {AnyDeals_home / 'cron/'}, {AnyDeals_home / 'sessions/'}, {AnyDeals_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ANYDEALS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Anydeals data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Anydeals code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in AnyDeals_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\AnyDeals\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(AnyDeals_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Anydeals-owned PATH entries in User environment")

        log_info("Removing ANYDEALS_HOME / ANYDEALS_GIT_BASH_PATH User env vars...")
        removed_env = remove_AnyDeals_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Anydeals-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing AnyDeals command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.AnyDeals/, just remove the AnyDeals-agent subdir
            if AnyDeals_home in project_root.parents or project_root.parent == AnyDeals_home:
    ...
    #     under ANYDEALS_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(AnyDeals_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(AnyDeals_home)
    ...
    # 5. Optionally remove ~/.AnyDeals/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ANYDEALS_HOME dirs live under
        ...
        try:
            if AnyDeals_home.exists():
                shutil.rmtree(AnyDeals_home)
                log_success(f"Removed {AnyDeals_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {AnyDeals_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {AnyDeals_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {AnyDeals_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/NousResearch/AnyDeals-agent/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/AnyDeals-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Anydeals Agent! ⚕")
    print()
