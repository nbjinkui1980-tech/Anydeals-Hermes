"""
Dump command for AnyDeals CLI.

Outputs a compact, plain-text summary of the user's Anydeals setup
...
from AnyDeals_cli.config import get_AnyDeals_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``AnyDeals dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from AnyDeals_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(AnyDeals_home: Path) -> int:
...
def _cron_summary(AnyDeals_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = AnyDeals_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"AnyDeals_home:      {display_AnyDeals_home()}")
    ...
    toolsets = config.get("toolsets", ["AnyDeals-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(AnyDeals_home)}")
    lines.append(f"  skills:             {_count_skills(AnyDeals_home)}")
