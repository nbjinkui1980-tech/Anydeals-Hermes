"""``AnyDeals logs`` — view and filter Anydeals log files.
...
Usage examples::
    ...
    AnyDeals logs                    # last 50 lines of agent.log
    AnyDeals logs -f                 # follow agent.log in real time
    AnyDeals logs errors             # last 50 lines of errors.log
    AnyDeals logs gateway -n 100    # last 100 lines of gateway.log
    AnyDeals logs --level WARNING    # only WARNING+ lines
    AnyDeals logs --session abc123   # filter by session ID substring
    AnyDeals logs --component tools  # only tool-related lines
    AnyDeals logs --since 1h         # lines from the last hour
    AnyDeals logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_AnyDeals_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Anydeals runs — try 'AnyDeals chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_AnyDeals_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_AnyDeals_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_AnyDeals_home()}/logs/")
        return

    print(f"Log files in {display_AnyDeals_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'AnyDeals chat' to generate logs)")
