"""
Profile management for multiple isolated Anydeals instances.

Each profile is a fully independent ANYDEALS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.AnyDeals/profiles/<name>/`` by default.

The "default" profile is ``~/.AnyDeals`` itself — backward compatible,
zero migration needed.

Usage::

    AnyDeals profile create coder          # fresh profile + bundled skills
    AnyDeals profile create coder --clone  # also copy config, .env, SOUL.md, skills
    AnyDeals profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    AnyDeals -p coder chat                 # or via flag
    AnyDeals profile use coder             # set as sticky default
    AnyDeals profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See AnyDeals_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.AnyDeals``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   AnyDeals-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `AnyDeals profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `AnyDeals
...
# `AnyDeals skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Anydeals infrastructure directories that only the default profile
         (``~/.AnyDeals``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_AnyDeals_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.AnyDeals) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "AnyDeals-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "AnyDeals", "default", "test", "tmp", "root", "sudo",
})

# Anydeals subcommands that cannot be used as profile names/aliases
_ANYDEALS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the AnyDeals root, NOT to the current ANYDEALS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ANYDEALS_HOME points outside
    ``~/.AnyDeals``, profiles live under ``ANYDEALS_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_AnyDeals_home() / "profiles"


def _get_default_AnyDeals_home() -> Path:
    """Return the default (pre-profile) ANYDEALS_HOME path.

    In standard deployments this is ``~/.AnyDeals``.
    In Docker/custom deployments where ANYDEALS_HOME is outside ``~/.AnyDeals``
    (e.g. ``/opt/data``), returns ANYDEALS_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_AnyDeals_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Anydeals installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ANYDEALS_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_AnyDeals_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, AnyDeals subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _ANYDEALS_SUBCOMMANDS:
        return f"'{canon}' conflicts with a AnyDeals subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``AnyDeals profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_AnyDeals_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``AnyDeals update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.AnyDeals)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.AnyDeals/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `AnyDeals update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`AnyDeals profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `AnyDeals update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ANYDEALS_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``AnyDeals profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ANYDEALS_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.AnyDeals).\n"
            "To remove everything, use: AnyDeals uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ANYDEALS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ANYDEALS_HOME")
    try:
        os.environ["ANYDEALS_HOME"] = str(profile_dir)
        from AnyDeals_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ANYDEALS_HOME"] = old_home
        elif "ANYDEALS_HOME" in os.environ:
            del os.environ["ANYDEALS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.AnyDeals/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: AnyDeals profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ANYDEALS_HOME.

    Returns ``"default"`` if ANYDEALS_HOME is not set or points to ``~/.AnyDeals``.
    Returns the profile name if ANYDEALS_HOME points into ``~/.AnyDeals/profiles/<name>``.
    Returns ``"custom"`` if ANYDEALS_HOME is set to an unrecognized path.
    """
    from AnyDeals_constants import get_AnyDeals_home
    AnyDeals_home = get_AnyDeals_home()
    resolved = AnyDeals_home.resolve()

    default_resolved = _get_default_AnyDeals_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.AnyDeals itself — its parent is ~/ and its
        # directory name is ".AnyDeals", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: AnyDeals profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.AnyDeals itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.AnyDeals). "
            "Specify a different name: AnyDeals profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="AnyDeals_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"AnyDeals.{old_name}"
    new_host = f"AnyDeals.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_AnyDeals_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for AnyDeals profile names."""
    return '''# Anydeals Agent profile completion
# Add to ~/.bashrc: eval "$(AnyDeals completion bash)"

_AnyDeals_profiles() {
    local profiles_dir="$HOME/.AnyDeals/profiles"
...
_AnyDeals_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_AnyDeals_profiles)" -- "$cur"))
        return
    fi
...
complete -F _AnyDeals_completion AnyDeals
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for AnyDeals profile names."""
    return '''#compdef AnyDeals
# Anydeals Agent profile completion
# Add to ~/.zshrc: eval "$(AnyDeals completion zsh)"

_AnyDeals() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.AnyDeals/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.AnyDeals/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ANYDEALS_HOME path string.

    Called early in the CLI entry point, before any AnyDeals modules
    are imported, to set the ANYDEALS_HOME environment variable.
    """
