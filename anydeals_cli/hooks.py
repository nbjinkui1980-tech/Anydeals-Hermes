"""AnyDeals hooks — inspect and manage shell-script hooks.

Usage::

    AnyDeals hooks list
    AnyDeals hooks test <event> [--for-tool X] [--payload-file F]
    AnyDeals hooks revoke <command>
    AnyDeals hooks doctor

Consent records live under ``~/.AnyDeals/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.AnyDeals/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``AnyDeals hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: AnyDeals hooks {list|test|revoke|doctor}")
        print("Run 'AnyDeals hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.AnyDeals/config.yaml.")
        print("See `AnyDeals hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `AnyDeals hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `AnyDeals hooks test` and `AnyDeals hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Anydeals wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `AnyDeals hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `AnyDeals hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `AnyDeals hooks doctor`.")
