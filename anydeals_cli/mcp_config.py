"""
MCP Server Management CLI — ``AnyDeals mcp`` subcommand.

Implements ``AnyDeals mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.AnyDeals/config.yaml under the ``mcp_servers`` key.
"""
...
from AnyDeals_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_AnyDeals_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from AnyDeals_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # AnyDeals_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  AnyDeals mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  AnyDeals mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  AnyDeals mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_AnyDeals_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: AnyDeals mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_AnyDeals_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `AnyDeals mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  AnyDeals mcp add <name> --url <endpoint>')
        _info('  AnyDeals mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `AnyDeals mcp remove` + `AnyDeals mcp add` to reconfigure auth.")
        return
...
# ─── AnyDeals mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'AnyDeals mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``AnyDeals mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("AnyDeals mcp serve                              Run as MCP server")
        _info("AnyDeals mcp add <name> --url <endpoint>        Add an MCP server")
        _info("AnyDeals mcp add <name> --command <cmd>         Add a stdio server")
        _info("AnyDeals mcp add <name> --preset <preset>       Add from a known preset")
        _info("AnyDeals mcp remove <name>                      Remove a server")
        _info("AnyDeals mcp list                               List servers")
        _info("AnyDeals mcp test <name>                        Test connection")
        _info("AnyDeals mcp configure <name>                   Toggle tools")
        _info("AnyDeals mcp login <name>                       Re-authenticate OAuth")
        print()
