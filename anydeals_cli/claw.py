"""AnyDeals claw — OpenClaw migration commands.

Usage:
    AnyDeals claw migrate              # Preview then migrate (always shows preview first)
    AnyDeals claw migrate --dry-run    # Preview only, no changes
    AnyDeals claw migrate --yes        # Skip confirmation prompt
    AnyDeals claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    AnyDeals claw migrate --no-backup  # Skip pre-migration snapshot
    AnyDeals claw cleanup              # Archive leftover OpenClaw directories
    AnyDeals claw cleanup --dry-run    # Preview what would be archived
"""
...
from AnyDeals_cli.config import get_AnyDeals_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Anydeals may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Anydeals gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Anydeals gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'AnyDeals stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_AnyDeals.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_AnyDeals", script_path)
...
def claw_command(args):
    """Route AnyDeals claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: AnyDeals claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Anydeals")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'AnyDeals claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Anydeals migration."""
    ...
    # migrate-AnyDeals posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: AnyDeals claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {AnyDeals_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.AnyDeals/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: AnyDeals claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Anydeals home ─────────
    # Delegates to AnyDeals_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `AnyDeals import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from AnyDeals_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(AnyDeals_home=AnyDeals_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: AnyDeals import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Anydeals home."
            )
    ...
    # 'AnyDeals claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: AnyDeals claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: AnyDeals claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  AnyDeals claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  AnyDeals claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  AnyDeals config set OPENROUTER_API_KEY sk-or-v1-...")
