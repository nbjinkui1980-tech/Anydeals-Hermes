"""Shell completion script generation for AnyDeals CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_AnyDeals_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Anydeals Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef AnyDeals
# Anydeals Agent zsh completion
# Add to ~/.zshrc:
...
_AnyDeals() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_AnyDeals_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'AnyDeals command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Anydeals Agent fish completion",
        "# Add to your config:",
        "#   AnyDeals completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __AnyDeals_profiles",
        "    echo default",
        "    if test -d $HOME/.AnyDeals/profiles",
        "        ls $HOME/.AnyDeals/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c AnyDeals -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c AnyDeals -f -s p -l profile"
        " -d 'Profile name' -xa '(__AnyDeals_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__AnyDeals_profiles)' -d 'Profile name'"
                )
