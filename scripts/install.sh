...
# Python-driven tool session (e.g. Anydeals terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/AnyDeals-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/AnyDeals-agent.git"
ANYDEALS_HOME="${ANYDEALS_HOME:-$HOME/.AnyDeals}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ANYDEALS_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ANYDEALS_INSTALL_DIR"
...
#   code at /usr/local/lib/AnyDeals-agent, command at /usr/local/bin/AnyDeals,
#   data still at /root/.AnyDeals (ANYDEALS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.AnyDeals/AnyDeals-agent"
            echo "                   default (root, Linux): /usr/local/lib/AnyDeals-agent"
            echo "  --AnyDeals-home PATH  Data directory (default: ~/.AnyDeals, or \$ANYDEALS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Anydeals installs the code under"
            echo "  /usr/local/lib/AnyDeals-agent and links the command into"
            echo "  /usr/local/bin/AnyDeals (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ANYDEALS_HOME"
            echo "  (default /root/.AnyDeals).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ANYDEALS_HOME/AnyDeals-agent are preserved in-place."
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Anydeals Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by JINKUI.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where the `AnyDeals` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ANYDEALS_HOME/AnyDeals-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ANYDEALS_HOME/AnyDeals-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/AnyDeals-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ANYDEALS_HOME/AnyDeals-agent — then preserve it)
#
# Always no-op when the user set --dir or $ANYDEALS_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ANYDEALS_HOME.
    if is_termux; then
        INSTALL_DIR="$ANYDEALS_HOME/AnyDeals-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ANYDEALS_HOME/AnyDeals-agent/.git" ]; then
            INSTALL_DIR="$ANYDEALS_HOME/AnyDeals-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/AnyDeals-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/AnyDeals-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/AnyDeals"
        log_info "  Data:    $ANYDEALS_HOME (unchanged)"
        return 0
    fi
...
get_AnyDeals_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/AnyDeals" ]; then
        echo "$link_dir/AnyDeals"
    else
        echo "AnyDeals"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/AnyDeals-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/AnyDeals-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/AnyDeals-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$ANYDEALS_HOME/node/bin/node" ]; then
        export PATH="$ANYDEALS_HOME/node/bin:$PATH"
        local found_ver=$("$ANYDEALS_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Anydeals-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.AnyDeals/node/..."
    ...
    # Place into ~/.AnyDeals/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$ANYDEALS_HOME/node"
    mkdir -p "$ANYDEALS_HOME"
    mv "$extracted_dir" "$ANYDEALS_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$ANYDEALS_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$ANYDEALS_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$ANYDEALS_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$ANYDEALS_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$ANYDEALS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.AnyDeals/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Anydeals install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Anydeals Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="AnyDeals-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Anydeals behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"AnyDeals-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up AnyDeals command..."

    if [ "$USE_VENV" = true ]; then
        ANYDEALS_BIN="$INSTALL_DIR/venv/bin/AnyDeals"
    else
        ANYDEALS_BIN="$(which AnyDeals 2>/dev/null || echo "")"
        if [ -z "$ANYDEALS_BIN" ]; then
            log_warn "AnyDeals not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$ANYDEALS_BIN" ]; then
        log_warn "AnyDeals entry point not found at $ANYDEALS_BIN"
    ...
    # Create a user-facing shim for the AnyDeals command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    cat > "$command_link_dir/AnyDeals" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ANYDEALS_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/AnyDeals"
    log_success "Installed AnyDeals launcher → $command_link_display_dir/AnyDeals"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "AnyDeals command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v AnyDeals' \
                >/dev/null 2>&1; then
        ...
        log_info "AnyDeals not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Anydeals Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Anydeals Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Anydeals Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so AnyDeals works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.AnyDeals directory structure (config at top level, code in subdir)
    mkdir -p "$ANYDEALS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.AnyDeals/.env (top level, easy to find)
    if [ ! -f "$ANYDEALS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ANYDEALS_HOME/.env"
            log_success "Created ~/.AnyDeals/.env from template"
        else
            touch "$ANYDEALS_HOME/.env"
            log_success "Created ~/.AnyDeals/.env"
        fi
    else
        log_info "~/.AnyDeals/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.AnyDeals/config.yaml (top level, easy to find)
    if [ ! -f "$ANYDEALS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ANYDEALS_HOME/config.yaml"
            log_success "Created ~/.AnyDeals/config.yaml from template"
        fi
    else
        log_info "~/.AnyDeals/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ANYDEALS_HOME/SOUL.md" ]; then
        cat > "$ANYDEALS_HOME/SOUL.md" << 'SOUL_EOF'
# Anydeals Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Anydeals communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.AnyDeals/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.AnyDeals/"

    # Seed bundled skills into ~/.AnyDeals/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.AnyDeals/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.AnyDeals/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ANYDEALS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$ANYDEALS_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.AnyDeals/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Anydeals itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (AnyDeals --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'AnyDeals setup' after install."
        return 0
    fi
    ...
    # Run AnyDeals setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m AnyDeals_cli.main setup < /dev/tty
    else
        python -m AnyDeals_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ANYDEALS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Anydeals to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ANYDEALS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'AnyDeals whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ANYDEALS_CMD="$(get_AnyDeals_command_path)"
                $ANYDEALS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'AnyDeals whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'AnyDeals gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ANYDEALS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ANYDEALS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: AnyDeals gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: AnyDeals gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ANYDEALS_CMD gateway > "$ANYDEALS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.AnyDeals/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: AnyDeals gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: AnyDeals gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ANYDEALS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ANYDEALS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ANYDEALS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}AnyDeals${NC}              Start chatting"
    echo -e "   ${GREEN}AnyDeals setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}AnyDeals config${NC}       View/edit configuration"
    echo -e "   ${GREEN}AnyDeals config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}AnyDeals gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}AnyDeals update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'AnyDeals' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'AnyDeals' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'AnyDeals' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
