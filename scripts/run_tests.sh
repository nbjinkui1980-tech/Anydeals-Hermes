#!/usr/bin/env bash
# Canonical test runner for AnyDeals-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.AnyDeals/AnyDeals-agent/venv"; do
...
# Unset ANYDEALS_* behavioral vars too.
unset ANYDEALS_YOLO_MODE ANYDEALS_INTERACTIVE ANYDEALS_QUIET ANYDEALS_TOOL_PROGRESS \
      ANYDEALS_TOOL_PROGRESS_MODE ANYDEALS_MAX_ITERATIONS ANYDEALS_SESSION_PLATFORM \
      ANYDEALS_SESSION_CHAT_ID ANYDEALS_SESSION_CHAT_NAME ANYDEALS_SESSION_THREAD_ID \
      ANYDEALS_SESSION_SOURCE ANYDEALS_SESSION_KEY ANYDEALS_GATEWAY_SESSION \
      ANYDEALS_CRON_SESSION \
      ANYDEALS_PLATFORM ANYDEALS_INFERENCE_PROVIDER ANYDEALS_MANAGED ANYDEALS_DEV \
      ANYDEALS_CONTAINER ANYDEALS_EPHEMERAL_SYSTEM_PROMPT ANYDEALS_TIMEZONE \
      ANYDEALS_REDACT_SECRETS ANYDEALS_BACKGROUND_NOTIFICATIONS ANYDEALS_EXEC_ASK \
      ANYDEALS_HOME_MODE 2>/dev/null || true
...
# If a system-wide AnyDeals pytest_live_guard plugin is installed at
# $HOME/.AnyDeals/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.AnyDeals/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.AnyDeals:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.AnyDeals" ;;
  esac
...
WORKERS="${ANYDEALS_TEST_WORKERS:-4}"
