...
# Bootstrap Open WebUI against Anydeals Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.AnyDeals/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-AnyDeals.sh
...
#   OPEN_WEBUI_NAME='Johnny Anydeals'
...
#   ANYDEALS_API_PORT=8642
#   ANYDEALS_API_HOST=127.0.0.1
#   ANYDEALS_API_MODEL_NAME='Anydeals Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Anydeals Agent WebUI}"
...
ANYDEALS_ENV_FILE="${ANYDEALS_ENV_FILE:-$HOME/.AnyDeals/.env}"
ANYDEALS_API_PORT="${ANYDEALS_API_PORT:-8642}"
ANYDEALS_API_HOST="${ANYDEALS_API_HOST:-127.0.0.1}"
ANYDEALS_API_CONNECT_HOST="${ANYDEALS_API_CONNECT_HOST:-127.0.0.1}"
ANYDEALS_API_MODEL_NAME="${ANYDEALS_API_MODEL_NAME:-Anydeals Agent}"
ANYDEALS_API_BASE_URL="http://${ANYDEALS_API_CONNECT_HOST}:${ANYDEALS_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-AnyDeals.sh"
LOG_DIR="$HOME/.AnyDeals/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ANYDEALS_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ANYDEALS_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.AnyDeals.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.AnyDeals</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.AnyDeals"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.AnyDeals"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-AnyDeals.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Anydeals Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-AnyDeals.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.AnyDeals/logs/openwebui.log
StandardError=append:%h/.AnyDeals/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-AnyDeals.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ANYDEALS_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Anydeals API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ANYDEALS_ENV_FILE"
  upsert_env API_SERVER_HOST "$ANYDEALS_API_HOST" "$ANYDEALS_ENV_FILE"
  upsert_env API_SERVER_PORT "$ANYDEALS_API_PORT" "$ANYDEALS_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ANYDEALS_API_MODEL_NAME" "$ANYDEALS_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ANYDEALS_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Anydeals gateway so API server settings take effect...'
  AnyDeals gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ANYDEALS_API_CONNECT_HOST}:${ANYDEALS_API_PORT}/health" >/dev/null; then
    log 'Anydeals API server did not answer on the first check. Trying to start gateway in the background...'
    nohup AnyDeals gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ANYDEALS_API_CONNECT_HOST}:${ANYDEALS_API_PORT}/health" >/dev/null
  ...
  log "Anydeals API endpoint: ${ANYDEALS_API_BASE_URL}"
