...
AnyDeals meet install                 # pip deps + Chromium (transcribe only)
AnyDeals meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
AnyDeals meet auth                    # optional; skips guest-lobby wait
AnyDeals meet setup                   # preflight checks
```

`AnyDeals meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or ANYDEALS_MEET_REALTIME_KEY in ~/.AnyDeals/.env
```
...
pip install playwright websockets && python -m playwright install chromium
AnyDeals plugins enable google_meet
AnyDeals meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
AnyDeals meet node approve my-mac ws://<mac-ip>:18789 <token>
AnyDeals meet node ping my-mac                   # confirm reachable
```

Run `AnyDeals meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Anydeals agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `AnyDeals meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ANYDEALS_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `AnyDeals_meet_src`). |
...
Local:
```
$ANYDEALS_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ANYDEALS_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `AnyDeals meet node approve`.
