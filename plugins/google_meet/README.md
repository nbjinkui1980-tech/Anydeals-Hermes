...
Let the AnyDeals agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where AnyDeals runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `AnyDeals meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `AnyDeals meet` CLI |
...
| `cli.py` | `AnyDeals meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ANYDEALS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `AnyDeals meet node {run,list,approve,remove,status,ping}` |
...
AnyDeals meet install                                      # pip + Chromium
AnyDeals meet setup                                        # preflight
AnyDeals meet auth                                         # optional
AnyDeals meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
AnyDeals meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.AnyDeals/.env
AnyDeals meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
AnyDeals meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
AnyDeals meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.AnyDeals/.env
AnyDeals meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, AnyDeals will **not** switch your system audio input automatically — the
...
AnyDeals plugins enable google_meet
AnyDeals meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
AnyDeals meet node ping my-mac
