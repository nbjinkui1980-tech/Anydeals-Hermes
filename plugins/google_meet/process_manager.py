...
session-scoped state file under ``$ANYDEALS_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ANYDEALS_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_AnyDeals_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this AnyDeals install, leave it first —
    ...
    env = os.environ.copy()
    env["ANYDEALS_MEET_URL"] = url
    env["ANYDEALS_MEET_OUT_DIR"] = str(out)
    env["ANYDEALS_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ANYDEALS_MEET_HEADED"] = "1"
    if auth_state:
        env["ANYDEALS_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ANYDEALS_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ANYDEALS_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ANYDEALS_MEET_MODE"] = mode
    if realtime_model:
        env["ANYDEALS_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ANYDEALS_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ANYDEALS_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ANYDEALS_MEET_REALTIME_KEY"] = realtime_api_key
