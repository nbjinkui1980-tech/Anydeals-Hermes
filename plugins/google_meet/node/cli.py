"""`AnyDeals meet node ...` subcommand tree.

Wired into the existing ``AnyDeals meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``AnyDeals meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="AnyDeals-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``AnyDeals meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             AnyDeals meet node approve <name> ws://<host>:{args.port} {token}")
        try:
