...
Config file: `$ANYDEALS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `AnyDeals` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `AnyDeals-{identity}` → `AnyDeals-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Anydeals profile:

```json
{
  "container_tag": "AnyDeals-{identity}"
}
```

For a profile named `coder`, this resolves to `AnyDeals-coder`. The default profile resolves to `AnyDeals-default`. Without `{identity}`, all profiles share the same container.
