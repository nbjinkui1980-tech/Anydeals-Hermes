...
_DEFAULT_CONTAINER_TAG = "AnyDeals"
...
def _load_supermemory_config(AnyDeals_home: str) -> dict:
    config = _default_config()
    config_path = Path(AnyDeals_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, AnyDeals_home: str) -> None:
    config_path = Path(AnyDeals_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `AnyDeals memory setup`.
        # All other options are documented for $ANYDEALS_HOME/supermemory.json
    ...
    def save_config(self, values, AnyDeals_home):
        ...
        _save_supermemory_config(sanitized, AnyDeals_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from AnyDeals_constants import get_AnyDeals_home
        self._AnyDeals_home = kwargs.get("AnyDeals_home") or str(get_AnyDeals_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._AnyDeals_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "AnyDeals", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "AnyDeals_memory", "target": target, "type": "explicit_memory"},
