...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/AnyDeals>
...
```bash
AnyDeals honcho setup    # full interactive wizard (cloud or local)
AnyDeals memory setup    # generic picker, also works
```

Or manually:
```bash
AnyDeals config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ANYDEALS_HOME/honcho.json` | Profile-local (isolated Anydeals instances) |
| 2 | `~/.AnyDeals/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Anydeals profile: `AnyDeals` (default) or `AnyDeals.<profile>`.
...
| 4 | `per-session` strategy | Anydeals session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`AnyDeals-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`AnyDeals`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-AnyDeals-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening AnyDeals in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Anydeals session ID (timestamp + hex). Every `AnyDeals` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Anydeals profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`AnyDeals`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Anydeals profile: `AnyDeals` (default) or `AnyDeals.<profile>` (e.g. `AnyDeals -p coder` → host key `AnyDeals.coder`).
...
| `ANYDEALS_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `AnyDeals honcho setup` | Full interactive setup wizard |
| `AnyDeals honcho status` | Show resolved config for active profile |
| `AnyDeals honcho enable` / `disable` | Toggle Honcho for active profile |
| `AnyDeals honcho mode <mode>` | Change recall or observation mode |
| `AnyDeals honcho peer --user <name>` | Update user peer name |
| `AnyDeals honcho peer --ai <name>` | Update AI peer name |
| `AnyDeals honcho tokens --context <N>` | Set context token budget |
| `AnyDeals honcho tokens --dialectic <N>` | Set dialectic max chars |
| `AnyDeals honcho map <name>` | Map current directory to a session name |
| `AnyDeals honcho sync` | Create host blocks for all Anydeals profiles |
