...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: AnyDeals)
...
Or via $ANYDEALS_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Anydeals can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ANYDEALS_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_AnyDeals_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "AnyDeals": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "AnyDeals"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Anydeals config."""
    profile = config.get("profile", "AnyDeals")
    return str(profile or "AnyDeals")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Anydeals profile name (from agent_identity)
      {workspace} — Anydeals workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``AnyDeals-{user}`` with no user becomes ``AnyDeals``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Anydeals Agent and the User"
    ...
    def save_config(self, values, AnyDeals_home):
        """Write config to $ANYDEALS_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(AnyDeals_home) / "hindsight"
    ...
    def post_setup(self, AnyDeals_home: str, config: dict) -> None:
        ...
        from AnyDeals_cli.memory_setup import _curses_select
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(AnyDeals_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "AnyDeals")
        ...
        self.save_config(provider_config, AnyDeals_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(AnyDeals_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(AnyDeals_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "AnyDeals"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: AnyDeals-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Anydeals Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "AnyDeals"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "AnyDeals"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "AnyDeals", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "AnyDeals")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Anydeals Agent and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See AnyDeals-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Anydeals created/used that client on the shared async loop,
