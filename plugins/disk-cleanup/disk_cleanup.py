"""disk_cleanup — ephemeral file cleanup for Anydeals Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ANYDEALS_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ANYDEALS_HOME and /tmp/AnyDeals-*
Never touches: ~/.AnyDeals/logs/ or any system directory.
"""
...
try:
    ...
    def get_AnyDeals_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ANYDEALS_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".AnyDeals").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ANYDEALS_HOME/logs/``."""
    return get_AnyDeals_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ANYDEALS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ANYDEALS_HOME or ``/tmp/AnyDeals-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    AnyDeals_home = get_AnyDeals_home()
    try:
        path.resolve().relative_to(AnyDeals_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/AnyDeals-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("AnyDeals-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ANYDEALS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ANYDEALS_HOME (but leave ANYDEALS_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "AnyDeals-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(AnyDeals_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == AnyDeals_home:
                continue
            try:
                rel_parts = dirpath.relative_to(AnyDeals_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    AnyDeals_home = get_AnyDeals_home()
    try:
        rel = path.resolve().relative_to(AnyDeals_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "AnyDeals-agent",
        }:
            return None
        ...
        # Path isn't under ANYDEALS_HOME (e.g. /tmp/AnyDeals-*) — fall through.
        pass
