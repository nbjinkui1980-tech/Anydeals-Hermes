...
Auto-tracks and cleans up ephemeral files created during Anydeals Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ANYDEALS_HOME` and `/tmp/AnyDeals-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ANYDEALS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ANYDEALS_HOME` or `/tmp/AnyDeals-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ANYDEALS_HOME/disk-cleanup/` is itself excluded
- `$ANYDEALS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
