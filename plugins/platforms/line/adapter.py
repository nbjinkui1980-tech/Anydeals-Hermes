"""
LINE Messaging API platform adapter for Anydeals Agent.
...
to Anydeals Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ANYDEALS_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            AnyDeals_home = Path.home().joinpath(".AnyDeals").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``AnyDeals status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``AnyDeals status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``AnyDeals setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.AnyDeals/.env`` via ``AnyDeals_cli.config``.
    """
    print()
    ...
    try:
        from AnyDeals_cli.config import get_env_var, set_env_var
    except ImportError:
        print("AnyDeals_cli.config not available; set LINE_* vars manually in ~/.AnyDeals/.env")
        return
