...
# ``model_tools`` import time, so ``AnyDeals status``, ``AnyDeals chat``, etc.
...
# ``AnyDeals_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Anydeals is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a AnyDeals MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ANYDEALS_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _AnyDeals_home = _get_AnyDeals_home()
        except (ModuleNotFoundError, ImportError):
            _AnyDeals_home = _Path.home() / ".AnyDeals"
        self._thread_count_store = _ThreadCountStore(
            _AnyDeals_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Anydeals is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ANYDEALS_HOME", str(_Path.home() / ".AnyDeals"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ANYDEALS_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a AnyDeals MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Anydeals is thinking…' marker message.
        ...
        "Anydeals is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Anydeals is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Anydeals is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Anydeals is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Anydeals is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``AnyDeals setup``.

    The setup wizard at ``AnyDeals_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.AnyDeals/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. AnyDeals-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.AnyDeals/.env")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Anydeals plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'AnyDeals-agent[google_chat]'",
        ...
        # the "Anydeals is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Anydeals is thinking…' marker message "
