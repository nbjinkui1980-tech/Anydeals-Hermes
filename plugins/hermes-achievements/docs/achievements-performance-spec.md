# Anydeals Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: AnyDeals-achievements plugin
...
### `GET /api/plugins/AnyDeals-achievements/achievements`
...
### `POST /api/plugins/AnyDeals-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/AnyDeals-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/AnyDeals-achievements/overview`
...
- Persist plugin data under:
  - `~/.AnyDeals/plugins/AnyDeals-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.AnyDeals/plugins/AnyDeals-achievements/scan_checkpoint.json`
...
- `~/.AnyDeals/plugins/AnyDeals-achievements/`
