# OpenAI-Compatible API Server for Anydeals Agent
...
SSE streaming. By exposing this endpoint, AnyDeals-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  AnyDeals-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.AnyDeals/.env`
2. Run `AnyDeals gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with AnyDeals-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns AnyDeals-agent as a model) |
...
Authorization: Bearer AnyDeals-api-key-here
...
But AnyDeals-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "AnyDeals-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured AnyDeals-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of AnyDeals-agent's core prompt
...
Once implemented, AnyDeals-agent works as a drop-in backend for:
