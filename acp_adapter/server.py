"""ACP agent server — exposes Anydeals Agent via the Agent Client Protocol."""
...
try:
    from AnyDeals_cli import __version__ as ANYDEALS_VERSION
except Exception:
    ANYDEALS_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Anydeals running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Anydeals cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Anydeals/OpenAI-compatible user content payload."""
...
class AnydealsACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Anydeals AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Anydeals version",
        },
    )
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from AnyDeals_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from AnyDeals_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Anydeals estimates ``used`` from the same
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["AnyDeals-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Anydeals using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="AnyDeals-agent", version=ANYDEALS_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Anydeals' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications; merely restoring server-side state makes Anydeals remember
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Anydeals on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Anydeals' local kawaii waiting/status
        ...
        # not the executor's. Also set ANYDEALS_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue ANYDEALS_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind ANYDEALS_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("ANYDEALS_INTERACTIVE")
            ...
            try:
                ...
                # Restore ANYDEALS_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("ANYDEALS_INTERACTIVE", None)
                else:
                    os.environ["ANYDEALS_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (ANYDEALS_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Anydeals Agent v{ANYDEALS_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Anydeals has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
