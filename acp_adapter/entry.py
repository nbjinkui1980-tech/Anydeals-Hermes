"""CLI entry point for the AnyDeals-agent ACP adapter.

Loads environment variables from ``~/.AnyDeals/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ANYDEALS_HOME (default ``~/.AnyDeals``)."""
    from AnyDeals_cli.env_loader import load_AnyDeals_dotenv

    AnyDeals_home = get_AnyDeals_home()
    loaded = load_AnyDeals_dotenv(AnyDeals_home=AnyDeals_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", AnyDeals_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting AnyDeals-agent ACP adapter")
    ...
    import acp
    from .server import AnydealsACPAgent
