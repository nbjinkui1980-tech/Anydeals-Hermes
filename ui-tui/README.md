...
React + Ink terminal UI for Anydeals. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ANYDEALS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.AnyDeals/.AnyDeals_history` or under `ANYDEALS_HOME`.
...
```text
ui-tui/
  packages/AnyDeals-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      AnyDeals-ink.d.ts    type declarations for @AnyDeals/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AnydealsCLI subprocess for slash commands
```
